...
- [x] [Review][Patch] Validate `ReadModelFreshnessThresholds` even when callers bypass `Create` [references/Hexalith.EventStore/src/Hexalith.EventStore.Client/Projections/ReadModelFreshnessThresholds.cs:21] — patched 2026-06-27: `ReadModelFreshness.Classify` now validates thresholds before classification, including values created via the public record-struct constructor, and regression tests cover direct negative-aging and stale-before-aging construction. Focused Client.Tests 480/480.
