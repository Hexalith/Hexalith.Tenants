...
> **This is still a plan, but the June 5 Story 1.0 spike clears the prior `FC-LYT` / `FC-CMD` / `FC-CNC` contract blockers, and Story 1.2 resolves the tenant-list `FC-TBL` boundary.** Each UI story must still satisfy its accessibility, localization, responsive, and documentation evidence gate.
