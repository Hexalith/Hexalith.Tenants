...
> Trust-first operations console for the Hexalith Tenants domain. Distilled from `.decision-log.md` + the PRD UX digest (`.working/prd-ux-digest.md`) + sources. The spines win on conflict with any mock or import; **EXPERIENCE.md wins on behavior**, `DESIGN.md` wins on visuals. This is a **PLAN, not blanket build-ready**: Story 1.0 cleared the `FC-LYT` / `FC-CMD` / `FC-CNC` readiness gates on 2026-06-05, and Story 1.2 resolved the `FC-TBL` tenant-list boundary with a Tenants-specific `TenantDataGrid`. Story-specific evidence gates still apply (see *FrontComposer Readiness & Fallbacks*). Synchronized 2026-07-19 with the approved [SCP-2026-07-15](../../sprint-change-proposal-2026-07-15.md): historical confirmations and completion statements are **historical evidence subject to reverification, never readiness waivers**; freshness-provenance-dependent behavior is gated on the PRD addendum §I work packages (PLAT-FRESH-1 / HOST-REF-1 / UI-READ-1), and whole-set search paging on SEARCH-CURSOR-1.
...
**Runtime & lifecycle constraint.** The app runs under **Blazor InteractiveServer with a server-side BFF** — the normative runtime (the original Blazor Auto assumption and the 2026-06-03 reconciliation note are superseded historical evidence). State must be **correct under at-least-once delivery and projection lag** (NFR-3): no prerender or circuit reconnect may resurrect a stale optimistic "success," and a reconnect re-derives truth from the projection, never from a cached in-flight assumption. Pending/in-flight indicators survive reconnect as *pending*, never silently promoted to confirmed. Typed immutable state is required; Fluxor is not a mandatory architecture constraint. The BFF owns the **safety assembly boundary** (PRD §10): it assembles and redacts every receipt, consequence-preview, and rejection view model, so rendered components receive only support-safe localized fields — never raw `NarrativePayload`, event bodies, command payloads, tokens, internal correlations, ETags, or raw metadata.
...
| **User lookup / user memberships** | **Users** tab (lookup/search-backed) · also a member row | Operator searches a user, views that user's tenant memberships (FR-4). Not an exhaustive all-users list. *(The former global-search entry point is removed — untraced to any requirement; reintroduction requires an explicit requirement and architecture decision, SCP-2026-07-15 P7.)* |
...
| **Audit Evidence Receipt** | Audit row · command result | Support-safe receipt for a recorded action — a BFF-assembled, redacted view model derived from `NarrativePayload` (FR-22). |
...
| **tenant-data-grid** | Cursor pagination only (never offset/limit). Required columns: tenant identity, status, member count, owner count, pending state, truth-state-badge with freshness. Search is server-side for non-empty terms: Memories supplies the match-set and the BFF hydrates authoritative rows. Search paging uses a **protected cursor** (SEARCH-CURSOR-1): the raw Memories offset is wrapped by the approved server-side cursor codec / DataProtection path and bound to the authenticated user plus normalized query, status, sort, direction, and page-size scope; neither the offset nor the protected cursor ever appears in visible copy, DOM attributes, logs, telemetry tags, or copy actions. On scope mismatch, decoding failure, or invalidation, search restarts from page 1 with the honest localized "list refreshed" notice; the internal offset advances by raw hits consumed — including dropped malformed, duplicate, unauthorized, or unhydrated hits (AD-10). Renders the **six non-collapsible list-surface states** plus a non-blocking search-unavailable notice when Memories is unavailable or degraded and the cursor list fallback is shown. **Sorting/paging must never hide a pending or stale marker** (GAP-4). **Row actions stay stable in width and placement** under data/sort/page change (GAP-9). On cursor invalidation, re-query page 1 with an honest "list refreshed" notice (not an error). |
...
| **audit-evidence-receipt** | Support-safe only: actor, target, tenant scope, outcome, timestamp, projection marker, audit/command reference — a view model **assembled and redacted by the server-side BFF** from `NarrativePayload` (target resolution `userId` → `key` → `TenantId`); the rendered component receives only support-safe localized fields. **Never** exposes raw payloads, event bodies, command payloads, tokens, internal correlation/message ids, ETags, raw event metadata, or PII. On partial completion shows the *actual* lifecycle state (e.g. `audit pending`) — **never** pre-rendered proof. |
...
| **primary-command-button** | The eligible primary affordance for *low-risk* command flows. Disabled when its gate is not `eligible`, with the reason inline. Aggregate-scoped locking (AD-12): disabled while a command for the **same aggregate** — (interactive circuit, `AggregateIdentity`) — is in flight; commands for unrelated aggregates remain available. |
...
**2. Freshness — 5 states:** `current` (usable), `refreshing` (usable nudge), `aging` (**usable with friction**), `stale` (**blocks** high-impact action), `unknown` (**blocks**, fails closed). Numeric `current`/`aging`/`stale` thresholds are deferred — **no magic numbers in this spine** (Open Q#10): they must be **configurable and honestly surfaced** (state badge + absolute timestamp; `unknown` when unmeasurable). Freshness carries **authoritative provenance** — ETag, projection version, and read-model freshness from the **six direct Tenants REST reads** (tenant list, tenant detail, tenant users, user tenants, tenant audit, global administrators); `ServedAt` is never a substitute for projection age. `refreshing` is client-transient (never claimed on the wire). On the unverified generic query route, provenance normalizes to `unknown` (fail-closed) — `current`/`stale` are not claims the interim transport may make; once PLAT-FRESH-1 / HOST-REF-1 / UI-READ-1 (PRD addendum §I) verify, the wire vocabulary is `current` / `stale` / `unknown`, and `aging` joins it only when authoritative projection-time provenance supports it.
...
- **Aggregate-scoped command locking (AD-12 — supersedes the global one-at-a-time FC-CNC scope):** lock scope is **(interactive circuit, `AggregateIdentity`)**. One command for the same aggregate stays active from submit through `accepted`/`projection pending` until terminal evidence; while it is, other command triggers **for that aggregate** are unavailable with a stated reason, and reconnect or failure never leaks or prematurely releases the lock. Commands for **unrelated aggregates may proceed.** Bulk submission, toast-batching, multi-row bulk actions, and multiple simultaneous commands for one aggregate remain prohibited in v1.
...
- **Copy-full-id (FR-7):** copies the **literal caller-supplied string** TenantId/UserId and any support-safe reference. **Never** `Guid.TryParse`/`Ulid.TryParse` — these ids are meaningful strings, not ULIDs/GUIDs. Cursors — including the protected Memories search-paging cursor (SEARCH-CURSOR-1) — are opaque/signed/scope-bound (never offset/limit); never copied as user-facing ids, never exposed in visible copy, DOM attributes, logs, or telemetry tags.
...
**Banned in v1:** optimistic success display; multiple simultaneous commands for one aggregate; bulk/multi-row command submission; toast-batching; hover-only disabled-action explanations; success announcement before projection confirmation; the words `undo`/`rollback`/`hidden edit`.
...
- **Ready-gate evidence set (reconcile-a11y Gap 1).** A UI story cannot be `ready` until it cites applicable **accessibility, localization, responsive, and documentation/reference (`FC-DOC`)** evidence — or records a Product/UX-approved row-specific fallback documenting five things: keyboard/focus/live-region behavior, localizable-copy responsibility, documentation/reference (`FC-DOC`) evidence, replacement path, and owner approval. **Required acceptance scenarios** (PRD §9's acceptance-evidence list is authoritative; this mirrors it): stale projection, rejected command, unknown confirmation, command accepted but still projection-pending, data unavailable but not authorization-denied, AD-12 command-lock retention through `accepted` and `projection_pending`, audit unavailable, last-owner warning, permission-missing; **plus keyboard-only complete-OR-exit of the consequence-preview and destructive-confirmation (focus trap, safe non-committing escape, focus return), forced-colors rendering of the status-icon set, and live-region politeness (assertive fires on rejection/failure/`unable to verify`/`degraded`/destructive-block; NO assertive on a resting destructive-control or a `risk high` badge).** Screen-reader review uses **NVDA + at least one browser/SR pairing**. Responsive evidence must include **horizontal table overflow, navigation collapse, and command-preview/dialog behavior at narrow widths**. Story 1.0 confirms the shell `FC-A11Y` capability, but keyboard/focus and forced-colors evidence still belongs to each implementing story.
...
- **CP-4 — SignalR nudge-only.** Live projection notifications are **freshness nudges only**; they prompt a re-query but **never** advance command lifecycle or audit availability. The projection is the source of truth. Correct under at-least-once delivery + projection lag (an InteractiveServer circuit reconnect re-derives, never resurrects optimistic success).
...
1. Sofia opens the **Audit trail list** (from a contextual entry — a tenant row, tenant detail, user lookup, or command result) and filters by date / `AuditEventCategory`.
...
|---|---|---|---|
| **FC-TBL** | Projection list/table (DataGrid, filter/search/empty/loading) | **available with resolved Tenants boundary** | Generated projection DataGrid does not satisfy Tenants cursor pagination, safety-column pinning, or six non-collapsing list states by itself. Story 1.2 uses a Tenants-specific `TenantDataGrid`; reusable cursor/pinning/list-state capability remains FrontComposer-owned enhancement work. |
| **FC-LYT** | Shell layout contract (full-width vs constrained) | **confirmed by Story 1.0 spike note, 2026-06-05** | Explicit full-width/constrained layout contract exists. UX intent remains full-width operational surfaces with constrained inner regions for forms/previews. |
| **FC-CMD** | Command-lifecycle feedback (three-phase, projection-confirmed) | **confirmed by Story 1.0 spike note, 2026-06-05** | Required for **ALL command FRs**. Maps to the `missing lifecycle support` unavailable-action-reason when unavailable in a specific surface. |
| **FC-A11Y** | Accessibility primitives | **confirmed by Story 1.0 spike note, 2026-06-05** | First-class, **non-removable even under fallback**. Per-story keyboard, focus, live-region, forced-colors, and screen-reader evidence remains required. |
| **FC-L10N** | Localization (shell resources) | **confirmed by Story 1.0 spike note, 2026-06-05** | Shell-owned versus Tenants-owned string boundary is confirmed; Tenants-owned domain copy still uses whole-string resources. |
| **FC-DOC** | Component documentation / Storybook | **confirmed by Story 1.0 spike note, 2026-06-05** | Equivalent docs exist; Storybook does not. Documentation/reference evidence remains part of the ready-gate evidence set. |
| **FC-CNC** | Concurrent-command / toast-batching policy | **confirmed by Story 1.0 spike note, 2026-06-05 — scope superseded by AD-12, 2026-07-15** | Lock scope is **aggregate-scoped per AD-12**: one active command per (interactive circuit, `AggregateIdentity`) from submit through terminal evidence; unrelated aggregates may proceed. Bulk submission, toast-batching, and multiple simultaneous commands for one aggregate remain prohibited. Story 1.0's global one-at-a-time policy is superseded historical evidence; the AD-12-scoped behavior requires reverification. |
...
1. **FC-AUD → flat audit DataGrid** — cursor-paginated, date + `AuditEventCategory` filters, states loading/empty/filtered-empty/error, in lieu of `<AuditTimeline>`; delivered for the Tenants-owned first slice by Epic 5 (historical evidence, subject to reverification).
...
3. **FC-CNC → one-at-a-time command policy** — approved 2026-06-03 as serialized single-command interaction; **scope since superseded (SCP-2026-07-15): locking is aggregate-scoped per AD-12** — one active command per (circuit, `AggregateIdentity`), unrelated aggregates proceed; bulk submission, toast-batching, and multi-row bulk actions remain prohibited.

**Readiness severity split.** Reversible high-impact lifecycle controls, specifically **FR-15 disable/enable**, are not classified as hard destructive tenant deletion. They were implemented in Epic 3 under the approved `FC-CNS` inline consequence-preview fallback and `FC-CNC` one-at-a-time command policy. Hard destructive tenant deletion remains out of scope as future administrators-only CLI tooling. **FR-19 global-administrator grant/remove** was implemented in Epic 4 with fixed-scope projection confirmation and last-admin safety; audit-based global-administrator correction was verified by Story 5.7 with the fixed `global-administrators` command scope and last-admin hard-stop safety. Until Story 5.7 is reverified under the corrected contract, global-administrator correction is unavailable with `high-impact flow not ready`. Tenant-scoped destructive actions — **FR-12 (remove-user)** and **FR-16/17 (config)** — were implemented in Epic 2/Epic 3 with story-specific fallback evidence, while reusable FrontComposer extraction and deferred correction-proof/freshness follow-ups remain tracked separately. All implementation statements in this section are **historical evidence, not readiness waivers** (SCP-2026-07-15): affected completed work must be reverified against the corrected story contracts, and readiness claims require the PRD addendum §I prerequisite work packages.

> **This remains a historical UX plan, but the June 5 Story 1.0 spike clears the prior `FC-LYT` / `FC-CMD` / `FC-CNC` contract blockers, Story 1.2 resolves the tenant-list `FC-TBL` boundary, Epic 2/Epic 3/Epic 4 deliver the command flows named above, and Epic 5 delivers the Tenants-owned flat audit/receipt/tenant-correction slice.** Per SCP-2026-07-15, these confirmations and deliveries are historical evidence subject to reverification against the corrected contracts — not readiness waivers. Each future UI story must still satisfy its accessibility, localization, responsive, and documentation evidence gate.

[NOTE FOR UX] Sources silent / deferred, flagged rather than invented: (1) numeric freshness thresholds — deferred to implementation + ops, must be configurable + surfaced (Open Q#10); (2) `duplicate` / `timeout` command-lifecycle tokens carry **no gloss** by spec instruction — none invented; (3) per-story localization copy ownership remains Tenants-owned even though the shell boundary is confirmed by Story 1.0; (4) FR-22/24/25 (receipt assembly + compensating recovery) now have Epic 5 implementation evidence for the Tenants-owned audit, receipt, availability, tenant-domain correction slice, and Story 5.7-verified audit-based global-administrator correction path; residual correction-proof/freshness and projection-pagination follow-ups remain tracked in `_bmad-output/implementation-artifacts/deferred-work.md`; (5) the pinned Fluent baseline is **`5.0.0-rc.4-26180.1`, centrally consumed** (SCP-2026-07-15 sweep; the earlier rc.2/rc.3 discrepancy is resolved history) — exact component/icon/ARIA behavior must be re-verified against the rc.4 package at build. **Owners for deferred obligations:** freshness thresholds → product/ops; contrast (WCAG 1.4.3 / 1.4.11) verification of the inherited Fluent roles → the implementing team's a11y reviewer at build; per-story a11y/l10n/doc evidence → the implementing story owner plus UX/a11y reviewer.
