...
> **Post-readiness update (2026-06-27; corrected 2026-07-15):** Story 1.0 completed the FrontComposer shell-integration spike and confirmed `FC-LYT`, `FC-CMD`, `FC-CNC`, `FC-A11Y`, `FC-L10N`, and `FC-DOC`; see `_bmad-output/implementation-artifacts/story-1-0-spike-note-2026-06-05.md`. Story 1.2 resolved the `FC-TBL` tenant-list grid decision with a Tenants-specific `TenantDataGrid`, and the 2026-06-21 Memories-backed search correction covers FR1 whole-set search. Per the approved [2026-07-15 readiness correction](./../../sprint-change-proposal-2026-07-15.md), completion statements are historical evidence, not readiness waivers: affected work must be reverified against the corrected contracts, and freshness-, search-, and production-dependent stories are gated on the addendum §I prerequisite work packages. Remaining gates are per-story accessibility/localization/responsive/documentation evidence, deferred numerics/freshness thresholds, `FC-TOK` fallback discipline, and audit/proof evidence readiness where applicable.
...
- **FrontComposer** provides the shell/layout/command groundwork to begin composing this UI. Story 1.0 confirmed the main shell contracts on 2026-06-05; Story 1.2 resolved the `FC-TBL` boundary by using a Tenants-specific `TenantDataGrid` for cursor pagination, safety-column preservation, and six non-collapsing list states while leaving reusable grid capability in FrontComposer — see the build-readiness status in §14.
...
- **Consequence Preview** — the pre-submission summary of what a command will and will not do, including **known consequences** and **known unknowns**, shown before destructive/high-impact actions. Assembled and redacted in the server-side BFF; its content set is defined in addendum §H.
...
- **Audit Evidence Receipt** — the support-safe record of a recorded action: actor, target, tenant scope, outcome, timestamp, projection marker, and an audit/command reference. Assembled and redacted in the server-side BFF from a structured **NarrativePayload** (never the raw event payload); rendered components receive only support-safe localized fields.
...
- **Rejection** — a business-rule refusal returned as a domain rejection event, surfaced through a BFF-assembled, support-safe rejection view model as safe, localized text — never a stack trace.
- **Proposed fallback** — a simpler implementation proposed for when a richer FrontComposer component is unavailable (e.g. a flat audit list instead of a timeline). A fallback is **only usable once Product/UX has approved it**. **The three interim fallbacks — `FC-AUD` → flat audit grid, `FC-CNS` → inline consequence text, `FC-CNC` → one-at-a-time commands — are approved** (recorded 2026-06-03; see the [Fallback Approval Record](./../../fallback-approval-record-2026-06-03.md)); each remains build-ready only once its other FrontComposer gates clear. The `FC-CNC` one-at-a-time scope is since superseded (2026-07-15): command locking is **aggregate-scoped per AD-12** (`architecture.md`) — one active command per (interactive circuit, AggregateIdentity) from submit through terminal evidence, while unrelated aggregates proceed; bulk submission, toast batching, and multiple simultaneous commands for one aggregate remain prohibited.
...
- **Notes:** uses the Product/UX-approved `FC-CNS` inline consequence fallback, the `FC-CMD` command-lifecycle contract, and the `FC-CNC` concurrency policy (aggregate-scoped per AD-12). Story 2.4 delivers the complete FR-12 vertical slice — fail-closed gating, consequence preview, elevated friction, projection-confirmed removal, audit-availability handling, and minimum support-safe removal proof (work package WP-2A, addendum §I). It has no dependency on Epic 5; Epic 5 generalizes audit browsing and recovery from this foundation, it does not complete FR-12 retroactively.
...
### 7.8 Audit Trail & Evidence *(ui-11, ui-12 — Phase 2c; flat-list fallback approved 2026-06-03; Epic 5 delivery is historical evidence)*
...
- **Consequences:** the receipt view model is assembled and redacted in the server-side BFF from a structured **NarrativePayload** (no new backend receipt endpoint); rendered components receive only support-safe localized fields and never raw `NarrativePayload`, event bodies, command payloads, tokens, internal correlations, ETags, or raw metadata; partial completion shows the actual lifecycle state (e.g. `audit pending`), never pre-rendered proof (CP-3).
...
*Post-Epic 5 / Epic 4 / Story 5.7 note (corrected 2026-07-15): compensating recovery (FR-24, FR-25) and evidence-receipt assembly (FR-22) carry Epic 5 implementation evidence (`epics.md` Stories 5.3, 5.5, and 5.6 plus the matching story records) — historical evidence, subject to reverification against the corrected story contracts. Stories 5.5–5.6 cover tenant-domain corrections only; Story 5.7 owns the complete global-administrator correction slice, and until it completes under the corrected contract, global-administrator correction is unavailable with `high-impact flow not ready`. Receipt and preview derivation occurs in the server-side BFF (FR-22). Residual follow-ups such as global-administrator projection pagination beyond the first page and story-specific routing assertions remain tracked in `_bmad-output/implementation-artifacts/deferred-work.md`.* `[NOTE FOR PM]`
...
**Feature-specific NFR (7.8/7.9):** audit rendering must meet the approved audit performance contract (§16.14 — a blocked Product/Operations decision record; Story 5.1 is not Ready before it is approved); if a flat render cannot meet it, virtualization or a stricter page size is the required fallback.

## 8. Cross-Cutting Non-Functional Requirements

- **NFR-1 Performance & freshness.** Reads use cursor pagination and conditional requests so unchanged data is cheap; freshness is surfaced, not hidden, and carries **authoritative provenance** — ETag, projection version, and read-model freshness from the six direct Tenants REST reads (PLAT-FRESH-1 / HOST-REF-1 / UI-READ-1, addendum §I); `ServedAt` is never a substitute for projection age. Targets: a tenant list/detail/member surface renders interactive in **≤ ~1s on a warm projection** for a typical tenant; the audit view's performance contract is a blocked Product/Operations decision record (§16.14) — no numeric audit budget is claimed until it is approved. `[ASSUMPTION]`
...
- **BFF assembly boundary:** the server-side BFF assembles and redacts every receipt, consequence-preview, and rejection view model. Rendered components receive only support-safe localized fields and never receive raw `NarrativePayload`, event bodies, command payloads, tokens, internal correlations, ETags, or raw metadata — and forbidden fields must be provably un-renderable, un-copyable, un-announceable, un-loggable, and un-serializable into component state.
...
- **FrontComposer (the platform UI framework this UI composes) — readiness matters; do not treat unconfirmed capabilities as given:**
  - **Confirmed by Story 1.0 (2026-06-05):** the application shell/layout (`FC-LYT`), command-lifecycle feedback (`FC-CMD`), the concurrent-command policy (`FC-CNC` — since aggregate-scoped per AD-12: one active command per (circuit, AggregateIdentity), unrelated aggregates proceed; the historical global one-at-a-time scope is superseded), localization resources (`FC-L10N`), accessibility primitives (`FC-A11Y`), and documentation/reference evidence (`FC-DOC`).
  - **Available with resolved Tenants boundary:** `FC-TBL` table/projection building block; generated projection DataGrid lacks Tenants-required cursor pagination, safety-column pinning, and six non-collapsing list states by itself. Story 1.2 resolves the Tenants implementation path with a Tenants-specific `TenantDataGrid`; reusable cursor/pinning/list-state capability remains FrontComposer-owned enhancement work.
...
- **Backend (consumes):** the **six direct Tenants REST reads** — tenant list, tenant detail, tenant users, user tenants, tenant audit, global administrators (addendum §C) — plus tenant lifecycle / member-role / configuration / global-administrator commands via `POST /api/v1/commands`. All UI reads route directly to Tenants with authoritative freshness provenance (UI-READ-1 / PLAT-FRESH-1 / HOST-REF-1, addendum §I); the generic EventStore query route is not a Tenants read path; commands and status lookup stay on the EventStore command client. The UI **composes** these with **custom command flows (not generated CRUD)**; it does not add backend endpoints (including receipt, preview, list-filter, or correction endpoints), reshape immutable domain contracts, or annotate command/query contracts for UI generation.
...
- **Hosting & orchestration boundary:** the Tenants UI host remains domain-owned; orchestration, shared hosting, health, telemetry, and production-operations ownership belong to a platform/composing host (PLATFORM-OPS-1, addendum §I) — the repository AppHost is transitional and is not expanded with shared platform plumbing. Production-readiness claims must not exceed recorded evidence; InteractiveServer stays at one replica until shared DataProtection, circuit/session routing, and cursor durability are verified.
...
- **R-1 Missing FrontComposer components block high-value flows.** `<AuditTimeline>` (`FC-AUD`) and `<ConsequencePreview>` (`FC-CNS`) do not exist; tokens (`FC-TOK`) remain missing as a shared capability. `FC-LYT`, `FC-CMD`, `FC-CNC`, `FC-A11Y`, `FC-L10N`, and `FC-DOC` are confirmed by Story 1.0 (2026-06-05). Story 1.2 resolved the Tenants `FC-TBL` implementation path with a Tenants-specific `TenantDataGrid`. **Mitigation:** MVP avoids `FC-AUD`/`FC-CNS` entirely (read-only); later phases use **Product/UX-approved fallbacks** (flat audit list, inline consequence text; approval recorded 2026-06-03 — see the [Fallback Approval Record](./../../fallback-approval-record-2026-06-03.md)), and the rich/reusable components are tracked as FrontComposer dependencies — not built inside Tenants.
...
- **R-3 Query cursor durability across replicas** is deferred (a separate backend epic); the UI must not assume cursors survive restarts/replica changes yet. **Mitigation:** treat cursors as opaque and session-scoped; on invalidation the UI restarts from page 1 with an honest localized notice (§16.7).
- **R-4 Fallback-approval dependency — RESOLVED (2026-06-03); contract confirmation updated 2026-06-27.** The three interim fallbacks (`FC-AUD`/`FC-CNS`/`FC-CNC`) are Product/UX-approved (see the [Fallback Approval Record](./../../fallback-approval-record-2026-06-03.md)); Story 1.0 confirmed `FC-LYT`, `FC-CMD`, `FC-CNC`, `FC-A11Y`, `FC-L10N`, and `FC-DOC`, and Story 1.2 resolved the `FC-TBL` tenant-list boundary — historical evidence, not readiness waivers (2026-07-15). Remaining gates are story-specific evidence, `FC-TOK` fallback discipline, audit/proof evidence readiness, and the addendum §I prerequisite work packages — not fallback approval or tenant-list grid decisioning.
...
> **Build-readiness status (updated 2026-07-15):** Story 1.0 completed the FrontComposer shell-integration spike and confirmed `FC-LYT`, `FC-CMD`, `FC-CNC`, `FC-A11Y`, `FC-L10N`, and `FC-DOC`. Story 1.2 resolved the `FC-TBL` tenant-list grid decision through a Tenants-specific `TenantDataGrid`, and the 2026-06-21 Memories-backed search correction covers FR1 whole-set search behavior. Story 2.1 records the first-command foundation evidence for the Tenants command gateway and projection-confirmed lifecycle. These are historical evidence, not readiness waivers (2026-07-15): freshness-, search-, and production-dependent stories are blocked on the addendum §I prerequisite work packages, and affected completed work must be reverified against the corrected contracts. Every UI story still needs accessibility/localization/responsive/documentation evidence; audit/proof UX remains gated by applicable evidence readiness. **This PRD is a complete plan, not a blanket green light for all stories.**

### 14.1 In scope (MVP — "Phase 2a")
- The **Operations Shell** with the single Tenants entry functioning (read): the Tenants and Users workspace tabs, plus the Global Administrators read surface reached contextually (§5.1).
...
- **Phase 2b — first command flows:** FR-10 (add user), FR-11 (change role), FR-13 (create tenant), FR-14 (edit metadata). *These are the most tractable command flows. Story 1.0 confirmed the command-feedback contract (`FC-CMD`) and the concurrent-command policy (`FC-CNC` — since aggregate-scoped per AD-12); Story 2.1 provides the first Tenants command gateway/lifecycle/projection-confirmation foundation evidence (historical evidence, subject to reverification against the corrected contracts). Future command stories still need to cite equivalent shared command lifecycle/truth/preview foundations and per-story evidence before they are ready.*
- **Phase 2c — high-impact, audit & recovery (gated on approved fallbacks, per-story evidence, and the addendum §I prerequisites):** FR-12 (remove user), FR-15 (disable/enable), FR-16–17 (configuration commands), FR-19 (global-admin commands), FR-20–23 (audit trail & evidence), FR-24–25 (compensating recovery). FR15 is a reversible lifecycle soft-delete / availability-control flow; FR-19 global-admin commands carry platform-governance blast radius and are gated on fixed-scope routing, freshness, last-administrator protection, and evidence gates — not categorically blocked. Epic 4 Stories 4.3 and 4.4 delivered them (historical evidence, subject to reverification against the corrected contracts).
- **Audit surface in MVP:** audit is a contextual route (§5.1), not a nav area; its list/evidence content is a Phase 2c deliverable. MVP shows the contextual entry as not-yet-available rather than a broken surface. `[ASSUMPTION]` — confirm whether to hide or stub the contextual audit entry in 2a (§16).

### 14.3 Phasing summary
`Phase 2a (MVP)` read-only foundation → `Phase 2b` first command flows → `Phase 2c` high-impact + audit + recovery. A backlog item carries `planning-only` (read, tenant-scoped, or approved reversible lifecycle-control fallback) or `blocked` (hard destructive platform-governance action, or dependent on a missing component with no approved fallback); it promotes to *ready* only when its FrontComposer dependencies resolve **or** a Product/UX-approved fallback is recorded. **The fallback-approval gate is satisfied** for the three interim fallbacks (recorded 2026-06-03 — see the [Fallback Approval Record](./../../fallback-approval-record-2026-06-03.md)); **the shell/layout/command contracts were confirmed** by Story 1.0 (2026-06-05) and **the tenant-list `FC-TBL` boundary resolved** by Story 1.2 — historical evidence, not readiness waivers (2026-07-15). Remaining promotion gates are story-specific evidence, `FC-TOK` fallback discipline, audit/proof evidence readiness, and the addendum §I prerequisite work packages (freshness provenance, split reads, protected search cursor, removal proof, production boundary). The 2026-06-06 correction reclassifies FR15 as eligible under the approved command and preview fallbacks; hard destructive tenant deletion remains out of scope for future administrators-only CLI tooling.
...
9. **Audit contextual entry in MVP (Phase 2a)** — hide it, or show a "not yet available" placeholder? (The IA itself is settled: audit is a contextual route, not a nav area — §5.1.)
...
14. **Audit performance contract (blocked decision record — Product/Operations owned; blocks Story 5.1 Ready).** Approve, before Story 5.1 is Ready: the representative 500-event dataset shape; page size and filter mix; reference environment and network assumptions; initial-render and interaction percentile budgets; the authoritative test tier and repeatability method; and the fallback trigger for stricter paging or virtualization. No numeric budget exists until this record is approved (§8 NFR-1, §7 feature NFR).
