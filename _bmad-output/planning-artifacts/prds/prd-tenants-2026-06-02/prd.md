...
> **Post-readiness update (2026-06-27):** Story 1.0 completed the FrontComposer shell-integration spike and confirmed `FC-LYT`, `FC-CMD`, `FC-CNC`, `FC-A11Y`, `FC-L10N`, and `FC-DOC`; see `_bmad-output/implementation-artifacts/story-1-0-spike-note-2026-06-05.md`. Story 1.2 resolved the `FC-TBL` tenant-list grid decision with a Tenants-specific `TenantDataGrid`, and the 2026-06-21 Memories-backed search correction covers FR1 whole-set search. Remaining gates are per-story accessibility/localization/responsive/documentation evidence, deferred numerics/freshness thresholds, `FC-TOK` fallback discipline, and audit/proof evidence readiness where applicable.
...
- **R-4 Fallback-approval dependency — RESOLVED (2026-06-03); contract confirmation updated 2026-06-27.** The three interim fallbacks (`FC-AUD`/`FC-CNS`/`FC-CNC`) are Product/UX-approved (see the [Fallback Approval Record](./../../fallback-approval-record-2026-06-03.md)); Story 1.0 confirms `FC-LYT`, `FC-CMD`, `FC-CNC`, `FC-A11Y`, `FC-L10N`, and `FC-DOC`; Story 1.2 resolves the `FC-TBL` tenant-list boundary. Remaining gates are story-specific evidence, `FC-TOK` fallback discipline, and audit/proof evidence readiness — not fallback approval or tenant-list grid decisioning.
...
> **Build-readiness status (updated 2026-06-27):** Story 1.0 completed the FrontComposer shell-integration spike and confirms `FC-LYT`, `FC-CMD`, `FC-CNC`, `FC-A11Y`, `FC-L10N`, and `FC-DOC`. Story 1.2 resolved the `FC-TBL` tenant-list grid decision through a Tenants-specific `TenantDataGrid`, and the 2026-06-21 Memories-backed search correction covers FR1 whole-set search behavior. Story 2.1 records the first-command foundation evidence for the Tenants command gateway and projection-confirmed lifecycle. Every UI story still needs accessibility/localization/responsive/documentation evidence; audit/proof UX remains gated by applicable evidence readiness. **This PRD is a complete plan, not a blanket green light for all stories.**
...
- **Phase 2b — first command flows:** FR-10 (add user), FR-11 (change role), FR-13 (create tenant), FR-14 (edit metadata). *These are the most tractable command flows. Story 1.0 confirms the command-feedback contract (`FC-CMD`) and one-at-a-time command policy (`FC-CNC`); Story 2.1 provides the first Tenants command gateway/lifecycle/projection-confirmation foundation evidence. Future command stories still need to cite equivalent shared command lifecycle/truth/preview foundations and per-story evidence before they are ready.*
