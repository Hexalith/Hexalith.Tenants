...
- **Terminal failure states reset to a fresh submittable preview on parent re-render (HIGH)** — `OnParametersSet` preservation guard omits `Failed/Rejected/UnableToVerify/Degraded`, so any parent re-render rebuilds the snapshot from `FromIntent`, discarding the rejection and re-enabling Submit; for UnableToVerify/Degraded it also destroys the tracking handle (loses the Refresh affordance). Shared by both panels; contradicts the non-collapse truth-state model. Fix: add the terminal failure states to the guard in both panels. (`GlobalAdministratorCorrectionPanel.razor:235`, `CorrectionStartPanel.razor:301`)
- **`ConfirmProjection` confirms off a known-Stale projection** — `ProjectionIsReadable` accepts `Kind ∈ {Ready,Stale,Empty}` and ignores `Freshness`, while the start gate requires `Freshness=Current`. Possibly intentional start-vs-confirm asymmetry; verify against the "Stale generally fails closed for mutations" rule. (`GlobalAdministratorCorrectionSnapshot.cs:327`)
...
- **Corrective-proof timestamp uses `CurrentCulture` instead of `InvariantCulture`** — UTC audit evidence should be culture-stable; harmless for EN/FR, latent elsewhere. Trivial fix. (`GlobalAdministratorCorrectionPanel.razor:216`)
