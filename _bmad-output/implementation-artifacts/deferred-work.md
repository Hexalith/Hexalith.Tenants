...
- **No story-specific gateway-routing test** — AC9/AC10 task asks to re-verify `Set/RemoveGlobalAdministratorAsync` routing stays fixed to `system/global-administrators/global-administrators`; the diff adds no `TenantCommandGatewayTests.cs` change, leaning on the unchanged 4.3/4.4 routing tests. Acceptable while the gateway is unchanged; add a focused routing assertion if the gateway is touched. (`tests/Hexalith.Tenants.UI.Tests/Services/Gateways/TenantCommandGatewayTests.cs`)
...
- **Corrective-proof lookup may link the wrong historical audit row** — **GLOBAL-ADMIN RESOLVED 2026-06-30; tenant-domain residual still open below.** The global-admin path now requires parseable invariant original timestamp evidence, requests system audit rows from that timestamp, filters strictly newer corrective rows, and reports audit delayed when the timestamp is missing/malformed. The tenant-domain `CorrectionStartPanel` still has the original matching-only proof lookup and is recorded under the 2026-06-30 deferred section. (`CorrectionStartPanel.razor:507`)
- **Focus call lacks `JSDisconnectedException` guard** — `OnAfterRenderAsync` → `FocusAsync()` has no try/catch; a dropped circuit/disposed panel between terminal `SetSnapshot` and render throws unhandled (the page already guards this). Pre-existing in the tenant panel (5.6). (`GlobalAdministratorCorrectionPanel.razor:246`, `CorrectionStartPanel.razor:312`)
- **Global-admin projection query unguarded in the page-load critical path** — `LoadAsync` awaits `RefreshGlobalAdministratorsProjectionAsync` with no try/catch; the gateway only swallows `EventStoreGatewayException`, so any other fault now breaks the whole audit page load. 5.7 wiring. (`TenantAuditPage.razor:334`)
- **~~Corrective-proof timestamp uses `CurrentCulture` instead of `InvariantCulture`~~ — RESOLVED 2026-06-30** — the proof *display* timestamp was fixed by the 2026-06-29 review (P9 — `ProofTimestampLabel` uses `InvariantCulture`); the live residual — the `originalTimestamp` *parse* in `WithCorrectiveProof` (and the panel's proof lookup) using ambient culture — was fixed in the 2026-06-30 code review by parsing with `CultureInfo.InvariantCulture` + `DateTimeStyles.RoundtripKind`. The same review also added a time tie-back so the corrective row must be at/after the original event time. (`GlobalAdministratorCorrectionSnapshot.cs` `WithCorrectiveProof`, `GlobalAdministratorCorrectionPanel.razor` `QueryCorrectiveProofAsync`). NB: the tenant-domain `CorrectionStartPanel` (story 5.6) still uses `CurrentCulture` in `ProofTimestampLabel` and lacks the proof time tie-back — see the 5.6 deferred items.
...
- **`CorrectionStartPanel` terminal-state focus parity (story 5.6)** — the tenant-domain correction panel moves keyboard focus only on `Confirmed`/`Failed`, not on `Rejected`/`Degraded`/`UnableToVerify`; the new global-admin panel handles all terminal states. Pre-existing in the 5.6 panel (this commit touches it only for the `ProjectionRefreshProvider` refactor). Trivial future parity fix mirroring `GlobalAdministratorCorrectionPanel.SetSnapshot`'s terminal-state set. (`src/Hexalith.Tenants.UI/Components/Tenants/Audit/CorrectionStartPanel.razor`)
...
- **Concurrent correction opens can finish projection refresh out of order** — `OpenCorrectionAsync` awaits a projection refresh before setting `_activeCorrectionIntent`; two rapid correction starts can complete out of order and leave the older intent active. This predates the dispatcher-marshalling change in `02e4dfb`. (`src/Hexalith.Tenants.UI/Components/Pages/TenantAuditPage.razor:450`)
- **Tenant-domain correction can still confirm from stale/degraded tenant detail** — the tenant projection refresh path strips snapshot freshness and passes only `TenantDetail` into `TenantCorrectionPreviewSnapshot.ConfirmProjection`, which confirms by tenant id and role without checking freshness. This is pre-existing tenant-domain behavior, separate from the global-admin freshness hardening. (`src/Hexalith.Tenants.UI/State/TenantAudit/TenantCorrectionPreviewSnapshot.cs:275`)
- **Tenant-domain corrective proof lookup can link unrelated historical rows** — the tenant panel still returns the first matching event type/tenant/target excluding the original event, with no timestamp or command tie-back. This mirrors the proof-causality gap addressed only partially for global administrators. (`src/Hexalith.Tenants.UI/Components/Tenants/Audit/CorrectionStartPanel.razor:508`)
