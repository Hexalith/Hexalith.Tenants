...
- **Corrective-proof lookup may link the wrong historical audit row** — `FirstOrDefault` matches by event-type + target + tenant, excluding only `OriginalAuditReference`, with no recency tie-back, so a prior historical event for the same user can be shown as proof. Shared pattern since 5.6. Fix: order by recency / match the just-submitted command timestamp. (`GlobalAdministratorCorrectionPanel.razor:382`, `CorrectionStartPanel.razor:507`)
...
- **Ledger-hygiene (see 5.7 patch P-9):** the 2026-06-29 entries "ConfirmProjection confirms off a known-Stale projection" and "Corrective-proof timestamp uses CurrentCulture" are now STALE — `ConfirmProjection` was hardened (P2) and the proof *display* timestamp was fixed (P9). The live residuals are the **pre-submit** preview gate accepting Stale and the proof **parse** spot, both reclassified as open patches in the 5.7 story file. Rewrite/close those two entries when the patches land.
