...
1. **Given** a developer reads `docs/quickstart.md`
   ...
   **Then** the guide starts with a prerequisite validation section checking: DAPR sidecar is running, EventStore is deployed, `system` tenant is configured in EventStore's domain service registration, and JWT claims include `eventstore:tenant` = `system`
    ...
    > **Spec note (superseded):** The DAPR sidecar check, EventStore deployment check, and `system` tenant JWT claim configuration are handled automatically by the Aspire AppHost topology. The implemented guide correctly omits these manual checks. The JWT token uses the `tenants` claim, which `EventStoreClaimsTransformation` maps to `eventstore:tenant` internally — this is correct behavior. AC 1 should be read as satisfied by the Aspire-managed topology.
...
4. **Given** the quickstart guide
   ...
   **Then** it includes: NuGet package installation, DI configuration, DAPR component setup reference, first command execution, and verification of the produced event
    ...
    > **Spec note (superseded):** "DAPR component setup reference" is not applicable for the local Aspire dev experience — the AppHost configures the Redis state store and DAPR sidecar components automatically. AC 4 is satisfied as implemented; no manual component YAML is required for the quickstart.
...
- [x] Task 1: Create `docs/quickstart.md` — prerequisite validation section (AC: #1, #2)
    - [x] 1.1: Write prerequisite list with validation commands for each (only 3 prerequisites needed with Aspire — EventStore deployment and `system` tenant are handled automatically by the AppHost topology):
        - .NET 10 SDK (`dotnet --version` → 10.x)
        - DAPR CLI and runtime (`dapr --version`, `dapr init` status)
        - Docker running (`docker info`)
    - [x] 1.2: Write remediation steps for each prerequisite failure with links to official docs (DAPR: https://docs.dapr.io/getting-started/, .NET: https://dot.net)
    ...
    - [x] 1.4: State explicitly that the 30-minute target (FR59) assumes prerequisites are already installed — the clock starts at "clone"
...
- [x] Task 3: Create `docs/quickstart.md` — first command section (AC: #3, #4)
    ...
    - [x] 3.4: Write CreateTenant command payload example for `POST /api/v1/commands`. Note: `aggregateId` and `payload.TenantId` MUST match — the aggregateId IS the managed tenant ID per the identity scheme (`system:tenants:{aggregateId}`). Verify the actual `CreateTenant` record fields by inspecting `src/Hexalith.Tenants.Contracts/Commands/CreateTenant.cs` — it takes `(string TenantId, string Name, string? Description)`:
        ...
        ```
    - [x] 3.5: Write event verification — show the expected successful response shape (`DomainServiceWireResult` with `IsRejection: false` and `Events` array containing a `TenantCreated` event). Then show how to verify via the query endpoint `GET /api/tenants/my-first-tenant`. Include a note: "If the query returns 404, wait a moment and retry — projections are eventually consistent. The read model may take a few seconds to process the event."
    - [x] 3.6: Add a note for developers running the quickstart a second time: "If you've run this before, BootstrapGlobalAdmin will return a rejection (GlobalAdminAlreadyBootstrapped) — this is correct behavior. Use a different aggregateId/TenantId for CreateTenant, or expect a TenantAlreadyExists rejection."
    - [x] 3.7: Write a follow-up "try more commands" section with AddUserToTenant example showing multi-step workflow (create tenant → add user → verify user list)

- [x] Task 4: Create `docs/quickstart.md` — "Next Steps" section (AC: #4)
    - [x] **IMPORTANT**: This section is a clearly separated "Next Steps" appendix AFTER the core quickstart flow, not part of the 30-minute path. The developer's first milestone is seeing TenantCreated — everything after that is optional follow-up.
    - [x] 4.1: Write NuGet package installation steps for consuming service integration:
    ...
    - [x] 4.2: Write DI configuration snippet using `AddHexalithTenants()` extension method
    - [x] 4.3: Reference existing `docs/idempotent-event-processing.md` for event handling patterns
    - [x] 4.4: Reference the sample consuming service at `samples/Hexalith.Tenants.Sample/` as a complete working example
...
- [x] Task 6: Update `README.md` with full project documentation (AC: #5)
    - [x] 6.1: Write project description — multi-tenant management for the Hexalith ecosystem using event sourcing, DAPR, and .NET Aspire
    - [x] 6.2: Add badges (only badges that resolve to real endpoints — no placeholders, no coverage badge until coverage reporting is wired up):
    ...
    - [x] 6.3: Write features list (tenant lifecycle, user-role management, global admin, configuration, event-driven integration, in-memory testing fakes)
    - [x] 6.4: Write NuGet packages table listing all 5 packages with descriptions
    - [x] 6.5: Add quickstart link pointing to `docs/quickstart.md` — include time estimate: "~15 minutes with prerequisites installed, ~45 minutes including first-time prerequisite setup"
    - [x] 6.6: Add placeholder for "aha moment" demo link/GIF (created in Story 8.3)
    - [x] 6.7: Write project structure overview (src/, tests/, samples/, docs/)
    - [x] 6.8: Write contributing section (branch naming conventions, PR process, test requirements, link to `.editorconfig`)

- [x] Task 7: Validation
    - [x] 7.1: Verify `docs/quickstart.md` file is well-formed markdown, all code blocks have language annotations, all links are valid relative paths
    - [x] 7.2: Verify `README.md` renders correctly (no broken badge URLs, proper heading hierarchy, no orphaned links)
    - [x] 7.3: Verify all command examples use the correct tenant/domain/aggregateId format matching the actual `appsettings.json` configuration (`system|tenants|v1` domain service registration)
...
- `ClaimsTenantValidator` requires `eventstore:tenant` claim matching target tenant (case-sensitive)
- Adapted JWT acquisition from Keycloak-based (EventStore pattern) to dev-signing-key-based (PowerShell + bash scripts)
- Verified `CreateTenant` record: `(string TenantId, string Name, string? Description)` from Contracts
- Verified `BootstrapGlobalAdmin` record: `(string UserId)` from Contracts
- Verified `AddUserToTenant` record: `(string TenantId, string UserId, TenantRole Role)` from Contracts
- Verified `TenantRole` enum: `TenantOwner=0, TenantContributor=1, TenantReader=2`
- All referenced file paths confirmed to exist (LICENSE, .editorconfig, ci.yml, samples/, docs/idempotent-event-processing.md)

### Completion Notes List

- Created `docs/quickstart.md` with prerequisite validation (3 prerequisites: .NET 10, DAPR, Docker), clone/build/run instructions, JWT token generation scripts (PowerShell + bash), BootstrapGlobalAdmin → CreateTenant command sequence, event verification via query endpoint, AddUserToTenant follow-up, Next Steps section with NuGet integration, and troubleshooting section
- Updated `README.md` with project description, 3 badges (CI, NuGet, License), features list, NuGet packages table (5 packages), quickstart link with time estimate, Story 8.3 placeholder, project structure overview, and contributing section
- Key deviation from story spec: JWT acquisition uses dev signing key scripts instead of Keycloak, because the Tenants AppHost does not include Keycloak in its topology (unlike EventStore). This is the correct behavior for the Tenants project.
- All 17 code blocks in quickstart.md have language annotations (bash, powershell, json, csharp)
- All relative links verified to exist
- Command payloads verified against actual C# record definitions

### File List

- `docs/quickstart.md` — **CREATED** — Quickstart guide with prerequisites, clone/build/run, JWT auth, command walkthrough, troubleshooting
- `README.md` — **MODIFIED** — Full project README replacing minimal placeholder

### Change Log

- 2026-03-19: Created `docs/quickstart.md` and updated `README.md` — Story 8.1 implementation complete
