...
14. **Memories handoff spec authored.** This story produces a written handoff (under `_bmad-output/` or the docs the team uses for submodule handoffs) specifying the `MemoriesIndexUpdate` contract, upsert-by-`(Tenant,AggregateId)` ingestion, attribute indexing + REST attribute filter, aggregate-id-in-hit mapping, and `tenants-index` registration. No `references/Hexalith.Memories/**` files are edited here. [O3; CLAUDE.md Submodule Policy]
...
- [x] **Task 1 — Package & submodule wiring** (AC: 1, 7)
  - [x] Add `ProjectReference`s from `src/Hexalith.Tenants.UI/Hexalith.Tenants.UI.csproj` to `references/Hexalith.Memories/src/Hexalith.Memories.Contracts/...` and `references/Hexalith.Memories/src/Hexalith.Memories.Client.Rest/...` (submodule **source** references like EventStore — not NuGet; do not touch `Directory.Packages.props`). Reference the `MemoriesIndexUpdate` contract from wherever the handoff places it (target: `Hexalith.Memories.Contracts`).
  ...
  - [x] Update `_bmad-output/project-context.md` consumed-submodule list to include `Hexalith.Memories` (and `Hexalith.PolymorphicSerializations` if pulled transitively). Submodule Policy: root-declared under `references/` only; never `--init --recursive`.
...
- **Submodules:** root-declared under `references/` only; never `--init --recursive`; never edit `references/Hexalith.Memories/**` or `references/Hexalith.FrontComposer/**` here (handoffs only).
...
- [Source: references/Hexalith.Memories/src/Hexalith.Memories.Client.Rest/MemoriesClient.cs:159] `SearchAsync`; `BuildSearchPath` ~540–579 (no structured filter); `SearchRequest.cs`; `IngestAsync` ~404 `[Experimental]`
- [Source: references/Hexalith.Memories/src/Hexalith.Memories.Contracts/V1/{SearchResult,ScoredResult}.cs] (`SourceUri` returned verbatim; `ContentSnippet` ≤200 chars)
- [Source: references/Hexalith.Memories/src/Hexalith.Memories.EventStore/{EventStoreIntegrationServiceCollectionExtensions.cs:33,TenantEventRoutingOptions.cs,EventIngestionController.cs}] ingestion path; `CloudEventToIngestionInputMapper.cs:78` (SourceUri=cloudevent.id); `DedupKeyBuilder.cs` (no upsert)
- [Source: references/Hexalith.Memories/src/Hexalith.Memories.AppHost/Program.cs] resource graph (Redis Stack, FalkorDB, statestore/pubsub/secretstore/llm, Memories Server `AddProject<Projects.Hexalith_Memories_Server>` launch profile `http`, ports 3500/50001)
- [Source: src/Hexalith.Tenants.Client/Handlers/TenantProjectionEventHandler.cs] + [Registration/TenantServiceCollectionExtensions.cs] topic `tenants.events`; [samples/Hexalith.Tenants.Sample/Program.cs] consumer template; [references/Hexalith.EventStore/.../EventPublisher.cs] `PublishEventAsync` + CloudEvents metadata
...
- `references/Hexalith.Memories/Directory.Build.props` — CheckSubmodules guard tolerant of the nested-in-Tenants layout
- `references/Hexalith.Memories/src/Hexalith.Memories.Contracts/V1/SearchIndexEntryChanged.cs` — NEW contract (upsert)
- `references/Hexalith.Memories/src/Hexalith.Memories.Contracts/V1/SearchIndexEntryRemoved.cs` — NEW contract (delete)
- `references/Hexalith.Memories/src/Hexalith.Memories.Contracts/V1/MemoriesJsonContext.cs` — register the two contracts in the source-gen JSON context
