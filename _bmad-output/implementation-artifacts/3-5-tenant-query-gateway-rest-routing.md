...
- [x] [Review][Defer] EventStore.Admin.Server still routes tenant queries through the retired `TenantProjectionRouting`/actor [references/Hexalith.EventStore/src/Hexalith.EventStore.Admin.Server/Services/DaprTenantQueryService.cs:209] — deferred, out of scope (submodule must not be modified).
