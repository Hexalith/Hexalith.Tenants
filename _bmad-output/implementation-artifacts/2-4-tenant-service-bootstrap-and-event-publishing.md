...
- [x] Task 1: Add EventStore.Hexalith.Tenants project reference and wire Program.cs (AC: #1, #7, #8, #9)
    - [x] 1.1 Add project reference to `Hexalith.EventStore.Hexalith.Tenants` in Hexalith.Tenants.csproj
    - [x] 1.2 Implement full Program.cs: `AddServiceDefaults()`, `AddDaprClient()`, `AddHexalith.Tenants()`, `AddEventStoreServer()`, `AddEventStore(typeof(TenantAggregate).Assembly)`, `UseEventStore()`, full middleware pipeline
...
HTTP POST /api/v1/commands (JWT-authenticated)
  └─ CommandsController.Submit()                    [EventStore.Hexalith.Tenants — auto-discovered]
     ├─ Extract JWT `sub` claim as userId
     ├─ Sanitize extension metadata
     └─ MediatR pipeline:
        ├─ LoggingBehavior                          [EventStore.Hexalith.Tenants.Pipeline]
        ├─ ValidationBehavior (FluentValidation)    [EventStore.Hexalith.Tenants.Pipeline]
        ├─ AuthorizationBehavior (JWT claims)       [EventStore.Hexalith.Tenants.Pipeline]
...
**Key insight:** The EventStore.Hexalith.Tenants project is a LIBRARY that provides all controllers, middleware, pipeline behaviors, and error handlers. By adding a project reference and calling `AddHexalith.Tenants()`, the Tenants Hexalith.Tenants inherits the full infrastructure.

### Program.cs Blueprint

```csharp
using Hexalith.EventStore.Client.Registration;
using Hexalith.EventStore.Hexalith.Tenants.Extensions;
using Hexalith.EventStore.Hexalith.Tenants.Middleware;
...
builder.Services.AddHexalith.Tenants();                                    // EventStore infrastructure
...
`AddEventStore()` without parameters uses `Assembly.GetCallingAssembly()` which returns Hexalith.Tenants assembly — NOT the Server assembly where aggregates live. You MUST pass the Server assembly explicitly:

```csharp
// WRONG — scans Hexalith.Tenants assembly, finds no aggregates
...
Add to `src/Hexalith.Tenants/Hexalith.Tenants.csproj`:

```xml
<ProjectReference Include="..\..\Hexalith.EventStore\src\Hexalith.EventStore.Hexalith.Tenants\Hexalith.EventStore.Hexalith.Tenants.csproj" />
```
...
- `AddHexalith.Tenants()` extension method (MediatR pipeline, FluentValidation, JWT auth, error handlers)
...
With this reference, the Tenants Hexalith.Tenants' existing NuGet packages (MediatR, FluentValidation, JWT Bearer) become transitively available through EventStore.Hexalith.Tenants but should be KEPT for explicit version control.
...
**No custom event publishing code is needed in the Tenants Hexalith.Tenants.**
...
```text
src/Hexalith.Tenants/Hexalith.Tenants.csproj (ADD EventStore.Hexalith.Tenants reference)
```

**DO NOT create:**

- Controllers — inherited from EventStore.Hexalith.Tenants via assembly scanning
- Middleware — inherited from EventStore.Hexalith.Tenants
- Pipeline behaviors — inherited from EventStore.Hexalith.Tenants
- Error handlers — inherited from EventStore.Hexalith.Tenants
...
- [Source: Hexalith.EventStore/src/Hexalith.EventStore.Hexalith.Tenants/Extensions/ServiceCollectionExtensions.cs — AddHexalith.Tenants()]
- [Source: Hexalith.EventStore/src/Hexalith.EventStore.Hexalith.Tenants/Program.cs — Reference Program.cs]
...
- **Review Resolution (2026-03-17)**: Resolved 6 code review findings (1 High, 2 Medium, 3 Low):
  - [P1/High] Bootstrap rejection detection was unreachable — `SubmitCommandHandler` throws `DomainCommandRejectedException` on rejection, so the status-reading code was dead. Fixed by catching `DomainCommandRejectedException` directly. Updated test to mock the real throw behavior. AC #3 now correctly met.
  - [P2/Medium] `IsProcessorMismatch` was catching legitimate state rehydration errors. Removed the `"Unable to rehydrate"` match so corrupt state errors propagate.
  - [P3/Medium] RFC 7807 test was missing `detail` and `status` assertions per AC #6. Added `details.Status.ShouldBe(409)` and `details.Detail.ShouldNotBeNullOrWhiteSpace()`.
  - [P4/Low] Removed duplicate `/process-command` endpoint (dead code, only `/process` is used by DAPR).
  - [P5/Low] Added `StartAsync_when_cancelled_propagates_OperationCanceledException` test for bootstrap service.
  - [P6/Low] Moved JWT signing key from `appsettings.json` to `appsettings.Development.json`. Base config now has empty key and `RequireHttpsMetadata: true`.
...
- `src/Hexalith.Tenants/Configuration/TenantBootstrapOptions.cs`
...
- `tests/Hexalith.Tenants.IntegrationTests/Hexalith.TenantsRuntimeIntegrationTests.cs`
...
- `src/Hexalith.Tenants/Hexalith.Tenants.csproj` (added EventStore.Hexalith.Tenants reference, added InternalsVisibleTo for IntegrationTests)
- `src/Hexalith.Tenants/Program.cs` (replaced skeleton with full pipeline and `/process` endpoint)
- `src/Hexalith.Tenants/Bootstrap/TenantBootstrapHostedService.cs` (catch DomainCommandRejectedException for bootstrap-already-completed)
- `src/Hexalith.Tenants/DomainProcessing/DomainServiceRequestHandler.cs` (skip only on "No Handle method found")
...
- `tests/Hexalith.Tenants.IntegrationTests/Hexalith.TenantsRuntimeIntegrationTests.cs` (added MessageId to CommandEnvelope/SubmitCommandRequest)
...
- `Hexalith.EventStore/src/Hexalith.EventStore.Hexalith.Tenants/ErrorHandling/DomainCommandRejectedExceptionHandler.cs`
- `Hexalith.EventStore/src/Hexalith.EventStore.Hexalith.Tenants/Extensions/ServiceCollectionExtensions.cs`
- `Hexalith.EventStore/src/Hexalith.EventStore.Hexalith.Tenants/Controllers/CommandsController.cs`
...
- 2026-03-17: Adversarial code review (3 layers: Blind Hunter, Edge Case Hunter, Acceptance Auditor). Found 6 patch issues (1 High, 2 Medium, 3 Low), 4 deferred, 8 rejected. All 6 patches applied. Build: 0 warnings, 0 errors. All tests pass (115 Server, 26 Contracts, 2 Runtime Integration). Story complete.
...
- Verified focused tests pass in the current workspace: `Hexalith.Tenants.Server.Tests` 11/11, `Hexalith.Tenants.IntegrationTests` 2/2 (`Hexalith.TenantsRuntimeIntegrationTests`), and `Hexalith.Tenants.IntegrationTests` 5/5 (`DaprEndToEndTests`).
