...
Completion note: Story context updated after Story 10.3A published the EventStore cancellation handoff at submodule commit `bcccd504`. Tenants implementation can now consume the approved EventStore APIs instead of re-deciding the contract shape. Code review 2026-05-19 closed all 7 patch findings and the 1 decision-needed item (submodule pointer bump sanctioned); 4 LOW-severity items deferred to follow-ups. Full Debug/no-restore solution test gate: 634 passed, 4 skipped, 0 failed.
...
Code review on 2026-05-19 against commit `c8246f6` (10.3b file scope). Three review layers ran in parallel: Blind Hunter (diff-only adversarial), Edge Case Hunter (diff + project access), Acceptance Auditor (diff + spec). 41 raw findings → 12 retained after dedup/triage; 15 dismissed; 4 deferred.

- [x] [Review][Decision] EventStore submodule pointer moved from `bcccd504` to `8d740e5a` (1-commit descendant: "fix(server): use weak ActorProxy in QueryRouter to fix list-tenants NRE"). Resolved 2026-05-19: sanctioned by Jerome — descendant is required for `list-tenants` query routing and is allowed by Task L67 / Dev Notes L87.
- [x] [Review][Patch] Add `cancellationToken.ThrowIfCancellationRequested()` between empty-index check and Success return in `HandleListTenantsAsync` [src/Hexalith.Tenants/Actors/TenantsProjectionActor.cs:520]. Applied 2026-05-19.
- [x] [Review][Patch] Add `cancellationToken.ThrowIfCancellationRequested()` between empty-data check and Success return in `HandleGetUserTenantsAsync` [src/Hexalith.Tenants/Actors/TenantsProjectionActor.cs:441]. Applied 2026-05-19.
- [x] [Review][Patch] Added theory `RoleSensitiveQuery_pre_cancelled_throws_OCE_not_domain_errorAsync` exercising all 5 role-sensitive query types and asserting `OperationCanceledException` is thrown with the expected token identity (not a `QueryResult` with `Forbidden`, `Tenant not found`, `Invalid cursor.`, etc.) [tests/Hexalith.Tenants.Server.Tests/Projections/TenantsProjectionActorTests.cs]. Applied 2026-05-19.
- [x] [Review][Patch] Added theory `RoleSensitiveQuery_pre_cancelled_with_malformed_user_throws_OCE_per_base_actor_precedenceAsync`. **Discovery during patch application**: AC9 cheap-validation precedence for pre-cancelled tokens is NOT externally observable at the actor boundary — `CachingProjectionActor.QueryAsync(envelope, token)` (the EventStore base class from 10.3A) calls `ThrowIfCancellationRequested` BEFORE delegating to `ExecuteQueryAsync`, so a pre-cancelled token throws OCE before the Tenants malformed-user → Forbidden short-circuit (TenantsProjectionActor.cs:73-83) can run. The Tenants in-actor precedence (Forbidden short-circuit precedes line 83 cancellation checkpoint) still applies for cancellations that fire DURING actor execution, just not for pre-cancelled callers. The theory now pins this base-class precedence as the regression-guard contract. Applied 2026-05-19. Task line 57 phrasing "when that precedence is already externally observable" covers this case — externally, OCE wins.
- [x] [Review][Patch] Added token-identity tests for all four remaining query handlers (`GetTenant_passes_received_token...`, `GetTenantUsers_passes_received_token...`, `GetUserTenants_passes_received_token...`, `GetTenantAudit_passes_received_token...`) — each asserts `Received(1).GetStateAsync(..., cancellationToken: cancellation.Token)` against the relevant DAPR state-store boundary [tests/Hexalith.Tenants.Server.Tests/Projections/TenantsProjectionActorTests.cs]. Applied 2026-05-19.
- [x] [Review][Patch] Added two cancellation tests to `GlobalAdministratorProjectionHandlerTests`: `ProjectAsync_WithPreCancelledTokenThrowsBeforeSaveAsync` (pre-cancelled, asserts no save) and `ProjectAsync_PassesCancellationTokenToSaveStateBoundaryAsync` (token identity at the DAPR `SaveStateAsync` boundary) [tests/Hexalith.Tenants.Server.Tests/Projections/GlobalAdministratorProjectionHandlerTests.cs]. Applied 2026-05-19.
- [x] [Review][Patch] Tightened `ListTenants_passes_received_token_to_projection_state_readsAsync` to deserialize and assert the payload contains the expected 2 tenants (closes the "passes via empty-index Success branch" wrong-reason path if mock token matching ever drifts) [tests/Hexalith.Tenants.Server.Tests/Projections/TenantsProjectionActorTests.cs]. Applied 2026-05-19.

Validation 2026-05-19: focused gates `dotnet test ... --filter FullyQualifiedName~TenantsProjectionActorTests|FullyQualifiedName~TenantProjectionHandlerTests|FullyQualifiedName~GlobalAdministratorProjectionHandlerTests` → 120 passed / 0 failed. Full solution gate `dotnet test Hexalith.Tenants.slnx --configuration Debug --no-restore` → 634 passed, 4 skipped (known), 0 failed.
- [x] [Review][Defer] LOW: Various cancellation-checkpoint placement inconsistencies (token check sometimes missing between auth and pagination in `HandleGetTenantUsersAsync`; `IsGlobalAdminAsync` checks token before but not after the DAPR read; `IsAuthorizedForTenantAsync` early-return when `Members.ContainsKey` is true does not observe token; `GlobalAdministratorProjectionHandler` `ThrowIfCancellationRequested` inside the foreach runs before the null-evt filter) — none change observable behavior; defer to a consistency-cleanup pass.
- [x] [Review][Defer] LOW: Cancellation taxonomy gap around post-state-read cursor decode in `HandleListTenantsAsync` (line 540), `HandleGetUserTenantsAsync` (line 471), `HandleGetTenantUsersAsync` (line 400-ish), `HandleGetTenantAuditAsync` (line 367-ish). Pre-existing structural ordering (state read → admin check → cursor decode) predates 10.3B; reordering to put cursor decode before state read would expand scope beyond this story and AC9 explicitly allows cursor as a cheap-validation guard. Track for a future cursor/state-ordering refactor.
- [x] [Review][Defer] LOW: `applyEvent` delegate signature does not accept a cancellation token — per-event checkpoint observes cancellation between events, but cannot interrupt inside `JsonSerializer.Deserialize(...)` for a heavy individual event. Bounded by event-payload size; not a correctness issue. Same pattern in `TenantAuditProjection.ProjectAuditEvents` (no internal cancellation) and the per-attempt retry conflict log in `TenantProjectionWritePolicy`.
- [x] [Review][Defer] LOW: Story File List drift — `_bmad-output/implementation-artifacts/10-3b-...md` and `sprint-status.yaml` are listed but not in the diff slice, while commit `c8246f6` bundled 10.4 conformance test files (`ProjectionWriteConformanceFixture.cs`, `ProjectionWriteConformanceIntegrationTests.cs`, `ProjectionWriteConformanceTests.cs`, `AspireTopologyFixture.cs`, samples/Program.cs, test-design artifacts) under a single commit titled "Add conformance tests for tenant projection write safety". 10.3b and 10.4 changes co-mingled in one commit complicates blast-radius reasoning for downstream operators. Informational; impacts story tracking and PR hygiene, not correctness.

Dismissed (15) — not retained: single-arg `ExecuteQueryAsync` overload forwarding `CancellationToken.None` (intentional source-compat per 10.3A Dev Notes line 79); speculative OCE-swallow-by-activity-scope claim (verified: catch block only re-throws); cursor-mis-mapping false-positives (AC9 explicitly allows cheap-validation precedence over cancellation); ASP.NET Core default cancellation→499 mapping; `ProjectionDispatcher` optional default token parameter; `_loggedOrphanMemberships` HashSet thread safety (DAPR actors single-threaded); `PaginateAuditEntries` LINQ lazy enumeration concern (bounded by `pageSize`); various test infrastructure micro-details (`ScriptedTenantProjectionStateStore.CancelAfterTrySave` last-writer-wins, `exception.CancellationToken.ShouldBe` fragility under linked tokens — spec explicitly requires identity).
