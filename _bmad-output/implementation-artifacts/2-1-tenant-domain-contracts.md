...
**Key mental model:** Commands are inputs (what a user wants to do). Events are outputs (what happened). Commands are NOT serialized into the event store — only events are. Commands flow through MediatR in Tenants; events flow through DAPR pub/sub to consuming services. This is why commands don't implement `IEventPayload` but events do.
