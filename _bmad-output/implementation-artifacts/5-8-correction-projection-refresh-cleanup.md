...
- `dotnet build tests/Hexalith.Tenants.UI.Tests/Hexalith.Tenants.UI.Tests.csproj -c Debug -m:1 -nr:false -p:UseHexalithProjectReferences=true`
- `tests/Hexalith.Tenants.UI.Tests/bin/Debug/net10.0/Hexalith.Tenants.UI.Tests -noLogo -noColor -parallel none`
...
- 2026-06-30 (code-review patch): Applied all 8 patch findings from the `939bebc..02e4dfb` adversarial review: command-tracking-aware live preview refresh, stricter global-admin corrective proof matching, missing regression coverage, and BMAD ledger/status cleanup. Debug build passed 0 warnings/0 errors; UI test executable fallback passed 852/852.
...
- 2026-06-30 - Applied all patch findings from the `939bebc..02e4dfb` adversarial review, added regression coverage, re-verified UI.Tests 852/852 green, and moved story/sprint status to done.
...
- [x] [Review][Defer→FIXED] `ConfirmProjection` can confirm off a known-Stale projection (`ProjectionIsReadable` ignores `Freshness`) [src/Hexalith.Tenants.UI/State/TenantAudit/GlobalAdministratorCorrectionSnapshot.cs:327] — RESOLVED 2026-06-30 for the global-administrator path. Confirm now requires a current Ready projection; the pre-submit gate also requires `Freshness=Current`, with mixed Ready+Stale coverage. The tenant-domain stale/degraded detail residual is tracked separately in the 2026-06-30 defer list below. [edge]
- [x] [Review][Defer→FIXED/GLOBAL] Corrective-proof lookup may link the wrong historical audit row as proof [src/Hexalith.Tenants.UI/Components/Tenants/Audit/GlobalAdministratorCorrectionPanel.razor:382; src/Hexalith.Tenants.UI/Components/Tenants/Audit/CorrectionStartPanel.razor:507] — RESOLVED 2026-06-30 for the global-administrator path with invariant `originalTimestamp` parsing, audit requests bounded from the original timestamp, a strict `row.Timestamp > originalTimestamp` filter, and delayed proof when the original timestamp is missing/malformed. The tenant-domain `CorrectionStartPanel` residual remains deferred in the 2026-06-30 review findings below. [blind]
...
- [x] [Review][Defer→FIXED] Corrective-proof timestamp formatted with `CurrentCulture` instead of `InvariantCulture` [src/Hexalith.Tenants.UI/Components/Tenants/Audit/GlobalAdministratorCorrectionPanel.razor:216] — RESOLVED 2026-06-30 for the global-administrator display and parse paths; display uses `InvariantCulture`, and proof parsing uses `InvariantCulture` plus `DateTimeStyles.RoundtripKind`. [blind]
...
## Review Findings (Adversarial Code Review 2026-06-30, `939bebc..02e4dfb`)

### Review Findings

- [x] [Review][Patch] FIXED 2026-06-30 — Same-intent live update cannot distinguish pre-submit states from terminal tracked states [src/Hexalith.Tenants.UI/Components/Tenants/Audit/CorrectionStartPanel.razor:308; src/Hexalith.Tenants.UI/Components/Tenants/Audit/GlobalAdministratorCorrectionPanel.razor:255] — both panels now live-update only snapshots without command tracking and include projection-driven `UnableToVerify` blocks; tracked terminal backend no-op `AlreadyApplied` remains non-submittable on parent re-render.
- [x] [Review][Patch] FIXED 2026-06-30 — Corrective proof matching remains non-causal and can still link unrelated audit rows [src/Hexalith.Tenants.UI/Components/Tenants/Audit/GlobalAdministratorCorrectionPanel.razor:432; src/Hexalith.Tenants.UI/State/TenantAudit/GlobalAdministratorCorrectionSnapshot.cs:329] — global-admin proof lookup now requires parseable invariant original timestamp evidence, requests audit rows from that timestamp, filters strictly newer corrective rows, and reports audit delayed instead of linking when the timestamp is missing/malformed. Command-id causality is not present in the audit row contract.
- [x] [Review][Patch] FIXED 2026-06-30 — Story status is split between story file and sprint status [_bmad-output/implementation-artifacts/sprint-status.yaml:99] — story header and sprint status now both say `done`.
- [x] [Review][Patch] FIXED 2026-06-30 — Story 5.8 still lists fixed global-admin findings as deferred [_bmad-output/implementation-artifacts/5-8-correction-projection-refresh-cleanup.md:145] — fixed global-admin stale-projection and proof timestamp/proof lookup entries are now marked resolved, with tenant-domain residuals split into the current defer list.
- [x] [Review][Patch] FIXED 2026-06-30 — Story 5.7 review ledger contains stale committed-state wording [_bmad-output/implementation-artifacts/5-7-global-administrator-correction-verification.md:249] — the 5.7 ledger now says `Patch (resolved)`, explains the `02e4dfb` committed state, and notes this follow-up validation reached 852/852.
- [x] [Review][Patch] FIXED 2026-06-30 — Gateway-null command-lock release has no regression test coverage [src/Hexalith.Tenants.UI/Components/Tenants/Configuration/RemoveTenantConfigurationFlow.razor:510; src/Hexalith.Tenants.UI/Components/Tenants/Lifecycle/TenantLifecycleCommandFlow.razor:442] — added focused gateway-null refresh tests for remove-configuration and lifecycle flows proving terminal `UnableToVerify` releases parent command activity.
- [x] [Review][Patch] FIXED 2026-06-30 — Stale projection guard lacks mixed-state test coverage [tests/Hexalith.Tenants.UI.Tests/State/GlobalAdministratorCorrectionSnapshotTests.cs:103] — added Ready-surface/Stale-freshness coverage for the pre-submit global-admin gate.
- [x] [Review][Patch] FIXED 2026-06-30 — Pre-submit live-update recovery lacks inverse transition coverage [tests/Hexalith.Tenants.UI.Tests/Components/GlobalAdministratorCorrectionPanelTests.cs:133] — added inverse transition coverage for untracked `AlreadyApplied`/last-admin blocks becoming submittable and tracked terminal `AlreadyApplied` staying non-submittable; tenant panel parity coverage was added for same-role and role-conflict recovery.
- [x] [Review][Defer] Concurrent correction opens can finish projection refresh out of order [src/Hexalith.Tenants.UI/Components/Pages/TenantAuditPage.razor:450] — deferred, pre-existing. `OpenCorrectionAsync` awaits a projection refresh before setting `_activeCorrectionIntent`; two rapid correction starts can complete out of order and leave the older intent active. The race predates this diff's dispatcher marshalling.
- [x] [Review][Defer] Tenant-domain correction can still confirm from stale/degraded tenant detail [src/Hexalith.Tenants.UI/State/TenantAudit/TenantCorrectionPreviewSnapshot.cs:275] — deferred, pre-existing. The tenant projection refresh path strips snapshot freshness and passes only `TenantDetail` into `ConfirmProjection`, which confirms by tenant id and role without checking freshness. This is the tenant-domain counterpart to the global-admin freshness hardening.
- [x] [Review][Defer] Tenant-domain corrective proof lookup can link unrelated historical rows [src/Hexalith.Tenants.UI/Components/Tenants/Audit/CorrectionStartPanel.razor:508] — deferred, pre-existing. The tenant panel still returns the first matching event type/tenant/target excluding the original event, with no timestamp or command tie-back. This mirrors the proof-causality gap addressed only partially for global administrators.
