...
- Confirmed current stale EventStore Admin routing claim remains false with `rg -n "ProjectionActorType|TenantProjectionRouting|TenantsProjectionActor" references/Hexalith.EventStore/src/Hexalith.EventStore.Admin.Server/Services/DaprTenantQueryService.cs` returning no matches.
