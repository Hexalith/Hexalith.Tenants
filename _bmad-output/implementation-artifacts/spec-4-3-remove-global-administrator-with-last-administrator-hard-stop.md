...
- [x] [Review][Patch] Unowned terminal dispatch completion drops Failed/Rejected/AlreadyApplied evidence and releases the lock, so a replacement cannot adopt the exact outcome [src/Hexalith.Tenants.UI/State/TenantCommands/TenantAggregateCommandAdmissionGate.cs:364]
- [x] [Review][Patch] Narrow-viewport CSS hides `.ga-correction-panel__mutation-initiation` without `::deep`, so the FluentButton same-id retry stays visible [src/Hexalith.Tenants.UI/Components/Tenants/Audit/GlobalAdministratorCorrectionPanel.razor.css:78]
- [x] [Review][Patch] Correction same-id retry that fails `TryBeginReconciliationDispatch` only calls `SynchronizeOwnedReconciliation()` and returns, with no busy reason or recovery [src/Hexalith.Tenants.UI/Components/Tenants/Audit/GlobalAdministratorCorrectionPanel.razor:1269]
- [x] [Review][Patch] `remove-focus-browser-guard` never installs or pins Chromium; iteration-6 proof depends on whatever `google-chrome`/`chromium` the runner happens to have [.github/workflows/story-guards.yml:37]
...
### Review Findings — 2026-09-23 Group B1 (page tests)

0 decision-needed, 13 patch, 3 defer, 21 rejected. Diff: `91d2335..0f490cb` limited to `tests/Hexalith.Tenants.UI.Tests/Components/GlobalAdministratorsPageTests.cs` (3,332 diff lines), reviewed against a clean `git archive` of `0f490cb`. B2 (other UI tests, ~3.6k lines) and B3 (Server/Integration/Contracts tests, ~1.3k lines) remain for follow-up runs.

- [x] [Review][Patch] `AdmissionWakeupCapturedForAttemptACannotQueryAttemptB` never enters the resume branch: `SynchronizeOwnedGrantReconciliation()` returns false because the lease reconciliation already equals the snapshot, so `resumeGrant` is null. Replacing the wakeup call with an argument-free `RefreshGrantStatusAsync()` fails only the source-text pin. Forcing the branch makes the test fail, because the status call runs before the swap. Rewrite it so attempt A is captured, B is swapped in before the status call, and no lookup uses B's message ID [tests/Hexalith.Tenants.UI.Tests/Components/GlobalAdministratorsPageTests.cs:5879]
- [x] [Review][Patch] The grant status/projection guards mask each other: the superseded-notification test swaps `_grantSnapshot` and `_grantAdmissionLease` together, so deleting `razor:3829`, `:3830` or `:3911` fails only the source-text test. Add held-grant status and projection lease-only tests, plus a snapshot-only status variant, matching the removal-side per-guard tests [tests/Hexalith.Tenants.UI.Tests/Components/GlobalAdministratorsPageTests.cs:648]
- [x] [Review][Patch] `RefreshGrantStatusAsync(activationSnapshot, activationLease)` checks the pinned attempt only on entry. After `await ReauthorizeAsync()` it re-reads the live `_grantSnapshot.CorrelationId`, calls `BeginGrantMutation()` (which cancels the replacement's in-flight grant work), and uses the two-argument core that reads the live snapshot. A wakeup captured for attempt A can therefore query and invalidate attempt B. Re-check both references after reauthorization and call the four-argument core with the activation pair [src/Hexalith.Tenants.UI/Components/Pages/GlobalAdministratorsPage.razor:3755]
- [x] [Review][Patch] Removal page scenarios required by the spec are absent. No removal test uses `PagedSnapshot`/`hasMore: true` for reconciliation (iteration-1 "page-level multi-page reconciliation", triage L264). No test opens the removal dialog and then goes unsafe (task "keep cancel visible after a live unsafe resize", AC "cancel/Escape dispatch nothing even after viewport invalidation"). No removal preflight, retry or requery test covers `Unauthorized`/`MissingPermission` collapse. No test sends a tracked-status `Rejected` with `LastGlobalAdministrator` (domain-race row). No partial, prefix or trailing-space acknowledgement is tested (iteration 1) [tests/Hexalith.Tenants.UI.Tests/Components/GlobalAdministratorsPageTests.cs:2192]
- [x] [Review][Patch] Negative and exact-count assertions run without a settle barrier: `await Task.Delay(50)` before "no redispatch" (:4914), `cut.Render()` then `StatusHandles.ShouldBeEmpty()` (:750), transient `WaitForAssertion(Count == 2)`/`WaitUntilAsync(Count == 3)` for "one lookup"/coalescing (:640, :4592), and `SetGlobalAdministratorCalls == 2` right after the adoption wait (:3226). A later duplicate lookup or dispatch passes. Settle through the renderer (`await cut.InvokeAsync(() => { })`) or a handler-completion signal, then re-assert the final count (iteration 5 "replace fixed-delay … assertions with condition-based waits") [tests/Hexalith.Tenants.UI.Tests/Components/GlobalAdministratorsPageTests.cs:4914]
- [x] [Review][Patch] The source-text pins search for signature literals containing `\n` plus 8 spaces (:5815, :5834, :5842, :5870), but `ReadGlobalAdministratorsPageSource` does not normalize line endings and `.editorconfig` sets `end_of_line = crlf`. A CRLF checkout or save (Windows, `core.autocrlf`, or Visual Studio honoring editorconfig) fails four tests. Apply `.ReplaceLineEndings("\n")` in the reader [tests/Hexalith.Tenants.UI.Tests/Components/GlobalAdministratorsPageTests.cs:5569]
- [x] [Review][Patch] `AssertGuardsInsideRendererCallback` takes the last callback opener before the mutation and any later `}).ConfigureAwait(false)`. A mutation moved after an already-closed callback therefore still passes, as long as guards appear in the earlier callback. Assert that no callback close lies between `callbackIndex` and `mutationIndex`. These pins are the only protection for several grant guards (see above) [tests/Hexalith.Tenants.UI.Tests/Components/GlobalAdministratorsPageTests.cs:7404]
- [x] [Review][Patch] `RealChromiumFocusValidatorStopsBeforeChromiumWhenServerStartupFails` runs the shell validator inside the ordinary UI suite. The script exits on missing `obj/project.assets.json` (L10) or missing Chromium (L58) before reaching the forced server failure (L108), so the test fails on any host without bash, python3, Chromium or a restored UI project. Skip when those prerequisites are absent (`Assert.SkipUnless`); the real browser run stays in `story-guards` [tests/Hexalith.Tenants.UI.Tests/Components/GlobalAdministratorsPageTests.cs:4004]
- [x] [Review][Patch] The same subprocess test does not kill the child when `WaitForExitAsync().WaitAsync(15s)` times out, because `using Process` only releases the handle. The bash, python server and browser are orphaned. Catch the timeout, call `process.Kill(entireProcessTree: true)`, then rethrow [tests/Hexalith.Tenants.UI.Tests/Components/GlobalAdministratorsPageTests.cs:4031]
- [x] [Review][Patch] `RendererReplacementAutomaticallyRedispatchesSameAmbiguousGrantId` dropped the click on "Retry delivery" and the same-ID third dispatch. Its own removed comment explained that a label-only check lets the handler's live-prerequisite guard no-op silently. Restore the enabled check, the click, and the same-ID dispatch count assertion on `finalReplacement` [tests/Hexalith.Tenants.UI.Tests/Components/GlobalAdministratorsPageTests.cs:3225]
- [x] [Review][Patch] `grantSubmit.IndexOf("if (IsGrantSubmitDisabled)")` is only compared with `ShouldBeLessThan`, so deleting the guard (`-1`) still passes. Assert the index is greater than -1 first [tests/Hexalith.Tenants.UI.Tests/Components/GlobalAdministratorsPageTests.cs:6120]
- [x] [Review][Patch] `leaseA.ShouldNotBeSameAs(leaseB)` always passes, because `leaseB` comes from a separately constructed gate. Delete it or replace it with an assertion on the original lease's state [tests/Hexalith.Tenants.UI.Tests/Components/GlobalAdministratorsPageTests.cs:711]
- [x] [Review][Patch] `acquired.ShouldBeFalse()` makes the following `if (replacementLease is not null)` cleanup unreachable. Delete the dead block [tests/Hexalith.Tenants.UI.Tests/Components/GlobalAdministratorsPageTests.cs:6652]
- [x] [Review][Defer] `Remove_rejection_keeps_last_confirmed_rows_without_success_or_member_copy` passes the expected substring as the gateway's raw rejection text, and the stub localization also contains it, so a broken `Remove.Status.Rejected.*` key mapping falls back to raw text and still passes [tests/Hexalith.Tenants.UI.Tests/Components/GlobalAdministratorsPageTests.cs:2332] — deferred: pre-existing (context-only lines in this range); fix by using neutral raw text and asserting the localized string.
- [x] [Review][Defer] Requery mismatch branch may release a live same-attempt lease [src/Hexalith.Tenants.UI/Components/Pages/GlobalAdministratorsPage.razor:4772] — deferred: maybe-false (medium if true). When `_removeSnapshot` advances to the same attempt's next state during a held requery, `RetainOrReleaseRemoveCompletion(projectionSnapshot, projectionLease)` sees the same lease and, for a terminal stale-basis projection, calls `TryReleaseTerminal`, unlocking while the UI still shows a non-terminal state. Settle with a page test that advances the same attempt during a held confirming requery and asserts the lease/UI pair.
- [x] [Review][Defer] Story gitlink guard fails (`scripts/validate-story-gitlinks.py` exit 1: Commons and PolymorphicSerializations undeclared; Builds/EventStore/FrontComposer/Memories SHAs misstated) [spec-4-3-remove-global-administrator-with-last-administrator-hard-stop.md:739] — deferred: already DW-347 (Chunk C).

Rejected:
- `false` CS1998 "async without await" build break — the current SDK does not emit CS1998 (verified with a `TreatWarningsAsErrors` probe build).
- `false` Private fields set off the dispatcher race the page — every write happens while the page's continuation is parked on a gate, and `TaskCompletionSource.SetResult` orders the handoff.
- `false` Bidi overrides/isolates are not tokenized — U+202A–U+202E and U+2066–U+2069 are `UnicodeCategory.Format`, which `ShouldTokenize` already encodes.
- `false` Commit `2d3d5f0` overclaims last-administrator coverage — "last-administrator remove" names this story's removal dialog, and the commit adds `RemoveEndSentinelFallsBackToLifecycleWhenPreviewHasDisappeared`.
- `false` Unqualified status/projection rows lack coverage — `GlobalAdministratorRemoveCommandSnapshotTests` covers zero-event Completed, unchanged version and still-present refusal; the page only forwards to `ConfirmProjection`.
- `false` Launcher restoration and Escape zero-dispatch are untested — `CancelLauncherFocusCompletionReturnsInsideANewerRemovalPreview` pins the exact launcher focus, and the Escape test asserts `RemoveGlobalAdministratorCalls == 0`.
- `false` The correlationless pending nudge is "silently dropped" — dropping it is correct (no correlation, no status lookup), and the test asserts that the slot is cleared.
- `low` The notification-during-requery scenario was replaced by reflection — notification-driven removal lookups are covered by the `CorrelatedRemove*`/`NotificationOnly*` tests.
- `low` A ZWJ identity can only be acknowledged by paste — subjects are Keycloak GUIDs; exact literal typing is the AC, and a visible-token input would add surface.
- `low` Accessible names tokenize ordinary spaces — this is deliberately injective for screen-reader users, and GUID subjects contain no spaces.
- `low` The string-presence validator test proves no behavior — the behavior is executed by the `story-guards` Chromium job.
- `low` `InvalidOperationException` is missing from module-disposal containment — no reachable path from `IJSObjectReference.DisposeAsync` was shown.
- `low` Null or empty localization-override pairs are untested — production uses `IsNullOrWhiteSpace`.
- `low` Remove lifecycle stubs use free-text `Rejected("rejected", …)` — those tests assert lifecycle, not copy; localization is covered by the deferred rejection finding.
- `low` The busy-lease retry test stops before recovery — recovery after single-flight completion is exercised by the ambiguous-retry tests.
- `low` The `WaitSleepJoin` pause may be reached before the nudge lock — the end state still fails under the old check-before-lock code; a hook would add production surface.
- `low` Hard-coded projection read counts — they intentionally pin the complete-population reads.
- `low` `JSRuntimeMode.Loose` in the identity-display test — that test asserts rendered text, not interop.
- `low` Self-removal copy and all ten facts are not asserted at page level — `TenantsBffCompositionTests` covers self-removal and fact readiness.
- `low` Completion-before-adoption and grant withdrawal during removal are not page-tested — covered by admission-gate tests and the `IsLocked` assertions.
- `low` `Cancelling_a_previewed_remove_does_not_cancel_in_flight_grant_status_tracking` no longer opens a removal preview — the mutual lock makes that scenario unreachable, so only the name is stale.

### 2026-09-23 — Review pass (post Group B1 repair)
- `[maybe-false]` `[defer, carried]` A stale same-attempt removal requery may release the live lease; the same `RetainOrReleaseRemoveCompletion` branch and uncertainty were recorded above at the Group B1 requery-mismatch finding. A held same-attempt state-advance test would settle it. The analogous grant branch has a message-ID check but no state-advance proof; the same test shape would settle that claim.
- `[medium]` `[patch]` Correction removal `ConfirmProjection` copies an unavailable or incomplete projection into `LastConfirmedProjectionEvidence` and derives count/presence from it; the remove branch at `GlobalAdministratorCorrectionSnapshot.cs:591-600` lacks the readable-complete guard present in the restore branch. Keep the last confirmed population when the new projection is unqualified.
- `[medium]` `[patch]` The new multipage removal test covers a target on page two, while the page-level suite lacks the complementary advanced complete multipage absence case. Add a positive confirmation case that checks every page and retains one dispatch.
- `[medium]` `[patch]` The unsafe-resize test checks enabled Cancel but invokes only Escape. Click Cancel after a narrow resize and assert closure, focus return, and no dispatch.
- `[maybe-false]` `[defer, carried]` The Chromium focus harness uses a handwritten Fluent custom element rather than the Blazor component host. This is the previously recorded DW-345 evidence gap; a rendered-host `activeElement` trace would settle it.
- `[low]` `[reject]` A non-executable `chrome` file in the Playwright cache could defeat the browser-test skip, but the runner itself checks executability and the ordinary restored CI environment supplies the pinned executable. A cross-platform executable probe in this optional subprocess prerequisite is more complexity than this rare failure warrants.
- `[medium]` `[defer, carried]` Audit Request permission and Escalate both route to `BackHref`; DW-352 already records the distinct-destination decision for the audit story.
- `[medium]` `[defer]` Audit Continue read-only can link to a hidden grid heading when filter validation suppresses `ShouldRenderRows`; `CanContinueReadOnlyForState` checks rows and state but omits filter validation. This belongs to the later audit surface in the baseline range.
- `[medium]` `[defer, carried]` The story gitlink validator fails because later dependency commits changed pointers outside this review repair. DW-347 and the prior Group A/B1 findings already record that failure.
- `[high]` `[defer, carried]` The `pushall` skill can commit from detached HEAD; the earlier review log records this later tooling defect inside the broad baseline range.
- `[high]` `[defer, carried]` The `pushall` skill can choose a stale local `main` over the remote default; the earlier review log records this later tooling defect.
- `[medium]` `[defer, carried]` The `pushall` skill does not validate exact commit-message candidates; the earlier review log records this later tooling defect.
- `[high]` `[defer, carried]` The `pushall` skill can push clean merges without build or test validation; the earlier review log records this later tooling defect.
- `[medium]` `[defer, carried]` Three review-prompt artifacts embed about 13 MB of duplicate diffs; earlier review passes already recorded this later artifact-hygiene issue.
- `[medium]` `[patch]` A canceled removal retry can throw from `ReauthorizeAsync` or the complete-population loader through `RefreshRemoveStatusAsync`; its `finally` clears single-flight but no cancellation catch completes the callback. Contain cancellation while keeping the retained attempt and lease.
- `[medium]` `[defer, carried]` Audit payloads with `HasMore` and no cursor can leave Next enabled but unable to advance. The earlier iteration-6 review already recorded this later Story 5.1 pagination issue.
