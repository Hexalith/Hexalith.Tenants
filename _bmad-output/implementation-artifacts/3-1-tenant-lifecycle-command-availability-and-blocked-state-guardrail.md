...
- Disabled tenant status is an eventually consistent availability signal. Most member/metadata/configuration commands targeting disabled tenants reject with `TenantDisabledRejection`; lifecycle enable is a separate global-admin operation. It remained UI-blocked by governance in Story 3.1 only; Story 3.2 later enabled the approved reversible lifecycle flow after the 2026-06-06 correction. [Source: `docs/compensating-commands.md#Tenant Lifecycle Correction`; `docs/idempotent-event-processing.md`; `_bmad-output/planning-artifacts/sprint-change-proposal-2026-06-06.md`]
