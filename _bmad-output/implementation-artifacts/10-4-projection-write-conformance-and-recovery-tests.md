...
1. Given Stories 10.1 and 10.2 have selected and implemented the projection write concurrency/retry policy, when conformance tests run against tenant detail, tenant index, and audit projection writes, then each projection proves it preserves all successfully processed events under concurrent update and replay conditions.
...
3. Given projection writes fail after retry exhaustion, when recovery behavior is tested, then the failure path does not claim success and leaves safe diagnostic evidence sufficient for replay or repair without logging payload bodies or user-controllable display values.
...
5. Given future tenant projection implementations opt into the conformance suite, when they supply the required projection fixture contract, then the same concurrency and recovery expectations can be reused without duplicating test logic.
6. Given a guarded save succeeds for one state key and a later key fails or is replayed, when recovery tests inspect the outcome, then tests assert no cross-key transactionality is claimed and replay/idempotency from Stories 10.1 and 10.2 prevents duplicate or lost entries.
7. Given the projection write helper or adapter exposes retry attempt boundaries, when deterministic tests simulate conflict-then-success and retry exhaustion, then exact read/save attempt counts, max-attempt behavior, and no-stale-instance reuse are verified.
8. Given audit entries are replayed or merged after conflict recovery, when duplicate `EventId` values are present, then tests assert persisted entries remain authoritative and distinct same-timestamp events stay ordered by `Timestamp` then `EventId`.
...
- [ ] Confirm implementation prerequisites before writing conformance tests. (AC: 1, 2)
  ...
  - [ ] If either story is not implemented, stop before adding speculative tests and return this story to backlog or blocked with the missing prerequisite named.
...
- [ ] Build a reusable projection write conformance fixture in the test project. (AC: 1, 5, 7)
  ...
  - [ ] Let each projection scenario declare state key category, initial state, externally updated reload state, incoming event batch, expected saved state, and expected attempt counts.
  - [ ] Make failure assertions reusable: no successful `ProjectionResponse`, safe diagnostic context, no event payload or tenant display data in captured logs, and no extra writes after the terminal failure.
...
- [ ] Keep scope boundaries explicit. (AC: 1-8)
  ...
  - [ ] Do not introduce distributed locks, queue redesign, schema migrations, repair commands, admin UI, or diagnostic query endpoints.
  - [ ] Production code changes are acceptable only if needed to expose existing internal helper behavior to the test assembly in the narrowest way, for example `InternalsVisibleTo`, and only after confirming no better existing test seam is available.
...
- For retry exhaustion tests, assert no successful projection result is returned and any captured diagnostic context uses safe categories rather than full state keys, tenant names, payload bodies, cursor payloads, or membership details.
