...
Historical matrix rows below are retained for planning traceability. Where a row says one of the superseded IDs is `needs-confirmation`, `missing`, or waiting on the Story 1.2 `FC-TBL` decision, read that row through this 2026-06-27 status note.
...
| Tenant Configuration | TenantOwner edits namespace-grouped key/value configuration with validation and confirmation feedback. | `GetTenantQuery` or configuration read model; `SetTenantConfiguration`; `RemoveTenantConfiguration`. | `FC-TBL`, `FC-LYT`, `FC-CMD`, `FC-CNC`, `FC-CNS`, `FC-TOK`, `FC-A11Y`, `FC-L10N`, `FC-DOC` | Planning-only until command lifecycle, batching, configuration editor layout, and fallback decisions are confirmed. | Read-only configuration display can consume `FC-TBL`. Remove-setting or high-impact setting changes need `FC-CNS` only if product/UX classifies the action as requiring consequence preview. | `FC-LYT`, `FC-CMD`, `FC-CNC`, `FC-CNS`, `FC-TOK`, `FC-A11Y`, `FC-L10N`, `FC-DOC` | `_bmad-output/planning-artifacts/ux-design-specification.md`; `_bmad-output/planning-artifacts/prd.md`; `references/Hexalith.FrontComposer/src/Hexalith.FrontComposer.Shell/Components/DataGrid` |
...
| Global Admin Management | Platform administrator reviews and changes global administrator access. | Global administrator commands `SetGlobalAdministrator` and `RemoveGlobalAdministrator`; global administrator projection/read evidence from completed backend scope. | `FC-TBL`, `FC-LYT`, `FC-CMD`, `FC-CNC`, `FC-CNS`, `FC-TOK`, `FC-A11Y`, `FC-L10N`, `FC-DOC` | Planning-only until command lifecycle, consequence preview, concurrent command policy, tokens, accessibility, and docs are confirmed. | Read-only global admin table may use `FC-TBL`. Remove-global-admin workflow requires `FC-CNS` or explicit product/UX fallback because platform access is high impact. | `FC-LYT`, `FC-CMD`, `FC-CNC`, `FC-CNS`, `FC-TOK`, `FC-A11Y`, `FC-L10N`, `FC-DOC` | `_bmad-output/planning-artifacts/ux-design-specification.md`; `_bmad-output/planning-artifacts/prd.md`; `references/Hexalith.FrontComposer/src/Hexalith.FrontComposer.Shell/Components/EventStore/FcPendingCommandSummary.razor` |
...
Disable Tenant Flow:
  ...
  blockedBy: [FC-CNS, FC-CMD, FC-CNC, FC-TOK, FC-A11Y, FC-L10N, FC-DOC]
  fallback: Approved FC-CNS inline consequence-preview fallback may be used once story-specific evidence is satisfied; no backend consequence endpoint is added by the fallback. Hard tenant deletion is future administrators-only CLI tooling.
```
...
| `FC-L10N` | command names, adopter-facing outcome copy | `references/Hexalith.FrontComposer/src/Hexalith.FrontComposer.Shell/Resources/FcShellResources.resx`; `references/Hexalith.FrontComposer/src/Hexalith.FrontComposer.Shell/Resources/FcShellResources.fr.resx`; `FcPendingCommandSummary.razor.cs` localizes summary templates. | `Hexalith.FrontComposer` plus Tenants UI story author | Localizable command names, success, already-applied, rejected, needs-review, overflow, degraded-connection, and requery/retry copy. | `needs-confirmation` | Stories must name shell-owned versus Tenants-owned strings and formatting evidence, or remain planning-only. Copy must be bounded, adopter-safe, and free of raw payloads or internal IDs. | FrontComposer resource files and pending-summary formatting; UX copy rules. | No | Include `FC-L10N` whenever command outcome, warning, retry, summary, or degraded-state copy appears. |
...
| Invalid message | Fail closed, avoid raw ID exposure, and preserve visible context for manual requery. | `FC-CMD`; FrontComposer. |
...
| --- | --- | --- |
| Create Tenant | Planning-only unless reduced command feedback fallback is approved. | `FC-CMD`, `FC-CNC`, `FC-A11Y`, `FC-L10N`, `FC-DOC` |
| Edit Tenant | Planning-only for command feedback; read-only detail remains separate. | `FC-CMD`, `FC-CNC`, `FC-A11Y`, `FC-L10N`, `FC-DOC` |
...
| Add User | Planning-only until pending identity, confirmation, accessibility, and copy are confirmed. | `FC-CMD`, `FC-CNC`, `FC-A11Y`, `FC-L10N`, `FC-DOC` |
| Remove User | Planning-only tenant-scoped destructive flow; fallback must name consequence and batching behavior. | `FC-CNS`, `FC-CMD`, `FC-CNC`, `FC-TOK`, `FC-A11Y`, `FC-L10N`, `FC-DOC` |
| Change Role | Planning-only until command feedback, role/status token, accessibility, and localization are confirmed. | `FC-CMD`, `FC-CNC`, `FC-TOK`, `FC-A11Y`, `FC-L10N`, `FC-DOC` |
...
| Set Global Administrator | Blocked/planning-only until command feedback and platform-access copy are confirmed. | `FC-CMD`, `FC-CNC`, `FC-A11Y`, `FC-L10N`, `FC-DOC` |
| Remove Global Administrator | Blocked because platform access removal is high impact. | `FC-CNS`, `FC-CMD`, `FC-CNC`, `FC-TOK`, `FC-A11Y`, `FC-L10N`, `FC-DOC` |
