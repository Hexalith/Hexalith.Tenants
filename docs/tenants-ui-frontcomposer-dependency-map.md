...
| Disable or enable tenant | Custom reversible lifecycle high-impact flow with ordered consequence preview, role/status impact, re-enable language, command feedback, degraded-state recovery, and audit-evidence expectations. | `FC-CNS`, `FC-CMD`, `FC-CNC`, `FC-TOK`, `FC-A11Y`, `FC-L10N`, `FC-DOC`, plus `FC-LYT` for layout. | `planning-only` / fallback-eligible after the approved 2026-06-06 correction; copy `blockedBy: [FC-TOK]` from `ui-13-disable-or-enable-tenant` unless newer story evidence clears it. |
...
| Disable Tenant | Planning-only / fallback-eligible after the 2026-06-06 correction; hard tenant deletion is out of scope. | `FC-CNS`, `FC-CMD`, `FC-CNC`, `FC-TOK`, `FC-A11Y`, `FC-L10N`, `FC-DOC` |
...
| Set Configuration | Planning-only unless classified low-impact and reduced feedback is approved. | `FC-CMD`, `FC-CNC`, `FC-A11Y`, `FC-L10N`, `FC-DOC` |
| Remove Configuration | Planning-only; add `FC-CNS` and `FC-TOK` if Product/UX classifies the key as high impact. | `FC-CMD`, `FC-CNC`, `FC-A11Y`, `FC-L10N`, `FC-DOC`; conditional `FC-CNS`, `FC-TOK` |
