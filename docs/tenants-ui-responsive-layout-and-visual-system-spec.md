...
- **Story 9.2** (`docs/tenants-ui-operations-shell-spec.md`) owns the four-area primary navigation (Tenants, Users, Global Administrators, Audit), the tenant list as default triage surface, the "command lifecycle is never a separate primary navigation model" rule, and the layout-stable `loading` state (§1, §2.4). Responsive collapse rules here preserve this information architecture and context-preservation contract; they do not introduce a new navigation model.
- **Story 9.1** (`docs/tenants-ui-frontcomposer-dependency-map.md`) owns the 10 fixed dependency IDs. `FC-LYT` (layout/responsive) is `needs-confirmation` — the FrontComposer shell layout exists (`Hexalith.FrontComposer/src/Hexalith.FrontComposer.Shell/Components/Layout`), but the full-width/constrained variant contract is unconfirmed. `FC-TOK` (semantic tokens) is `missing` for timeline/consequence tokens; existing Fluent/FrontComposer status/role **badges** (`.../Components/Badges`) are adjacent evidence usable only as a named fallback. Reuse these IDs verbatim; add none.
...
**Explicitly forbidden for dense access-review workflows:** marketing-style card dashboards and hero-scale type. Tenants is an operational console, not a marketing site or card dashboard. Layout-variant decisions (full-width vs constrained) tie to `FC-LYT` (`needs-confirmation`) and require product/UX approval or a FrontComposer story that confirms the contract. [Source: `_bmad-output/planning-artifacts/ux-design-specification.md#Spacing & Layout Foundation`; `#Implementation Approach`; `docs/tenants-ui-frontcomposer-dependency-map.md#FC-LYT`]
...
Deferred Decision: "Approve full-width/constrained screen layout contract or row-specific current-shell fallback" (affected rows `ui-01` through `ui-15`). Every candidate row already carries `FC-LYT` in its `blockedBy` array.
...
- Identity/rejection sanitation applies to any visible label: never render raw payloads, bearer tokens, stack traces, internal correlation IDs, internal exception text, or PII. User-facing rejection text is composed at the HTTP boundary by EventStore's domain-rejection ProblemDetails handling/catalog (RFC 7807). IDs are ULIDs; use JWT `sub` for actor identity. [Source: `_bmad-output/project-context.md#API Surface`; `#Rejection Event Payloads`; `#Identity Scheme`; `#Logging & Telemetry`]
