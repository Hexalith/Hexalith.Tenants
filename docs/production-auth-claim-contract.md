...
| `eventstore:permission` (query path) | `query:read`, `queries:*`, the legacy `command:query`, or an exact query type token (for example `list-tenants`) | Authorizes the query routing path. |
