...
- Rejection events carry structured data only; user-facing text is composed at the HTTP boundary by `RejectionToHttpStatusMapper` (RFC 7807). Domain rejections map to safe, localized UI text — never expose raw payloads, stack traces, tokens, or internal exception text. [Source: `_bmad-output/project-context.md#API Surface`; `#Form Patterns`]
