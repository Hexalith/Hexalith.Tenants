...
DAPR topic scoping is component metadata: `publishingScopes` lists which app IDs may publish to which topics, `subscriptionScopes` lists which app IDs may subscribe to which topics, a blank topic list denies that app, and an app omitted from a scope list keeps DAPR's default broad access unless protected by other metadata. EventStore is the only publisher and is intentionally left unlisted in `publishingScopes`, so it keeps unrestricted publish access. This is required because EventStore provisions per-tenant topics dynamically (`{tenant}.{domain}.events`); pinning it to an explicit allow-list would silently deny any topic outside the list (the EventStore platform documents this as NFR20, and listing `eventstore` is the documented anti-pattern). Subscribers are denied publishing instead.

Local development intentionally omits topic-level pub/sub scopes in `src/Hexalith.Tenants.AppHost/DaprComponents/pubsub.yaml`; component-level `scopes` cover `eventstore`, `sample`, and the local-only Memories search-index subscriber (`memories-server`, which subscribes to `tenants.events`), but local demos remain flexible while developers iterate. Production denies `sample` publishing with an empty topic list (`publishingScopes: "sample="`) and allows `sample` to subscribe to `tenants.events` (`subscriptionScopes: "sample=tenants.events"`) in `deploy/dapr/pubsub.yaml`, while leaving `eventstore` unlisted for unrestricted publishing.

The local and production components intentionally differ only in topic-scope strictness, the local-only `memories-server` component scope, and environment-specific Redis connection settings. They share the same component name (`pubsub`), Redis component type, the `eventstore` and `sample` component scopes, and application-level dead-letter model; the local component additionally scopes `memories-server` for the search-index demo.
