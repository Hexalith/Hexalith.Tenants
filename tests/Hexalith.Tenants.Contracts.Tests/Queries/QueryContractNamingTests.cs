...
public class QueryContractNamingTests
{
    ...
    private static List<Type> GetQueryContractTypes()
    {
    ...
    private static string GetStaticProperty(Type type, string propertyName)
    {
        ...
        prop.ShouldNotBeNull($"Type '{type.Name}' is missing static property '{propertyName}'");
        object? value = prop.GetValue(null);
        value.ShouldNotBeNull($"Type '{type.Name}'.{propertyName} returned null");
        return (string)value;
    }
    ...
    public void QueryType_values_are_unique()
    {
        ...
        List<string> queryTypes = types.Select(t => GetStaticProperty(t, "QueryType")).ToList();
