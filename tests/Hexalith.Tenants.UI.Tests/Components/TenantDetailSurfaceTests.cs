...
public sealed class TenantDetailSurfaceTests : BunitContext
{
    ...
    public void Detail_page_loads_through_gateway_and_renders_operational_overview()
    {
        ...
        JSRuntimeInvocationHandler tenantWrite = module.SetupVoid("writeText", "tenant.alpha").SetVoidResult();
        JSRuntimeInvocationHandler memberWrite = module.SetupVoid("writeText", "owner-user").SetVoidResult();
        ...
        tenantWrite.Invocations.Single().Arguments[0].ShouldBe("tenant.alpha");
        ...
        memberWrite.Invocations.Single().Arguments[0].ShouldBe("owner-user");
    }
