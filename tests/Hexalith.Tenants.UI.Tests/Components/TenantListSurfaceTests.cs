...
public sealed class TenantListSurfaceTests : BunitContext
{
    ...
    public void Sparse_authoritative_search_page_keeps_next_paging_available_without_backfill()
    {
        ...
        RegisterServices(call =>
        {
            ...
            return Task.FromResult(request.SearchCursor == "protected-page-two"
                ...
                : TenantListSnapshot.FilteredEmpty() with
                {
                    NextCursor = "protected-page-two",
                    HasMore = true,
    ...
    public void Missing_retained_search_page_on_detail_return_restarts_page_one_with_polite_notice()
    {
        ...
        cut.Find("[data-testid='tenants-list-search-refreshed-notice']").GetAttribute("aria-live").ShouldBe("polite");
        cut.Find("[data-testid='tenants-list-previous']").HasAttribute("disabled").ShouldBeTrue();
    }
    ...
    public void Search_fallback_cursor_recovery_renders_both_non_blocking_support_safe_notices()
    {
        ...
        cut.WaitForElement("[data-testid='tenants-list-search-unavailable-notice']");
        cut.WaitForElement("[data-testid='tenants-list-refreshed-notice']");
        cut.Find("[data-testid='tenants-list-search-unavailable-notice']").GetAttribute("aria-live").ShouldBe("polite");
        cut.Find("[data-testid='tenants-list-refreshed-notice']").GetAttribute("aria-live").ShouldBe("polite");
    ...
    [InlineData(true)]
    [InlineData(false)]
    public void Workspace_recovery_clears_only_the_applicable_search_paging_history(bool recoverAuthoritative)
    {
        int callCount = 0;
        RegisterServices(_ =>
        {
            ...
            {
                return Task.FromResult(AuthoritativeSnapshot(
                    [Row("tenant.initial", "Initial", TenantStatus.Active, ReadModelFreshnessState.Unknown, TenantPendingState.Unknown)]));
            }

            TenantListSnapshot recovery = recoverAuthoritative
                ? AuthoritativeSnapshot(
                    [Row("tenant.authoritative", "Authoritative", TenantStatus.Active, ReadModelFreshnessState.Unknown, TenantPendingState.Unknown)]) with
                {
        ...
        paging.TryMovePrevious(!recoverAuthoritative).ShouldBeTrue();
    }

    [Fact]
    public void Unsupported_secondary_notice_reason_does_not_render_a_blank_message_bar()
    {
        RegisterServices(ReadySnapshot(
            [Row("tenant.alpha", "Alpha", TenantStatus.Active, ReadModelFreshnessState.Unknown, TenantPendingState.None)]) with
        {
            PagingNotice = TenantListReason.GatewayUnavailable,
        });
    ...
    public void Partial_authoritative_search_keeps_verified_rows_and_whole_set_semantics()
    {
        ...
        cut.Find("[data-testid='tenants-list-page-local-semantics']").TextContent.ShouldContain("whole authorized tenant index");
        cut.FindComponents<FluentSelect<string, string>>()
            ...
            .Label.ShouldBe("Exact status across search results");
        cut.Markup.ShouldContain("Pending state unknown");
    }
    ...
    private sealed class StubTenantsLocalizer : IStringLocalizer<TenantsResources>
    {
        ...
        private static readonly Dictionary<string, string> Values = new(StringComparer.Ordinal)
        {
            ...
            ["Tenants.List.StatusFilterLabel.Authoritative"] = "Exact status across search results",
            ["Tenants.List.PageLocalSemantics"] = "Status filters and column sorting apply to the current authorized page only.",
            ["Tenants.List.AuthoritativeSearchSemantics"] = "Search, status, and sorting apply to candidates from the whole authorized tenant index; displayed rows come from current authorized tenant reads.",
