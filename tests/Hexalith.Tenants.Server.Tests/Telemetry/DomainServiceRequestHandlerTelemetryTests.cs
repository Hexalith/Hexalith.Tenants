...
public class DomainServiceRequestHandlerTelemetryTests : IDisposable
{
    ...
    public async Task ProcessAsync_Success_ShouldEmitSpanWithCorrectTags()
    {
        ...
        processor.ProcessAsync(Arg.Any<CommandEnvelope>(), Arg.Any<object?>())
        ...
        await handler.ProcessAsync(CreateRequest("CreateTenant", "tenant-1"));
        ...
        activity.GetTagItem(TenantActivitySource.TagCommandType).ShouldNotBeNull();
    ...
    public async Task ProcessAsync_Success_ShouldRecordCommandDurationMetric()
    {
        ...
        (string Name, double Value, KeyValuePair<string, object?>[] Tags) metric =
    ...
    public async Task ProcessAsync_WithUnknownCommandType_ShouldSanitizeMetricDimension()
    {
        ...
        await handler.ProcessAsync(CreateRequest("UnknownMaliciousCommand", "tenant-1"));
