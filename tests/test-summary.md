...
- [x] Story 2.1 membership command-foundation reverify tests cover AggregateIdentity-shaped admission (`TenantAggregateCommandAdmissionGateTests`), owner-aware page leases including refusal of a second command surface and its inability to release a lease it never held (`TenantDetailSurfaceTests`), provenance-qualified confirmation with ordered projection-version advancement plus the state-store token contract and its opaque/GUID/hash fail-closed boundary (`TenantMembershipCommandProvenanceTests`), message-id reuse versus deliberate new attempts and retention of the minted id across indeterminate submissions for add-member, change-role and remove-member (`TenantCommandGatewayTests`), SignalR nudge forwarding that re-queries authoritatively without confirming from the notification, and support-safe feedback when an attempt's tracking is lost.
